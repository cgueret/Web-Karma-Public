...
public class Binding {
    ...
    /**
     * stores corresponding vars/vals that will become Assign operators in the
     * query graph.
     */
     ...
     * (non-Javadoc)
     * 
     * @see java.lang.Object#clone() Returns a clone of the current Binding.
     */
    ...
    /**
     * Adds key/value pair to bindingList Example: unify(x,y) => (x,y)
     * unify(x,1) => (x,y), (y,1)
     * 
     * @param key
     * @param val
	...
	while (existingVal != null) {
		...
		if (valIsConstant && !val.equals(existingVal)) {
		    // unification fails
		    System.err.println("Unification failed for:(" + key + ","
			    + val + ") in " + bindingList);
		    return false;
		} else {
		    // do nothing
    ...
    /**
     * @param key
     * @return return the constant corresponding to key (from
    ...
    /**
     * @param key
     * @return return unification var/val corresponding to key (from
     *         bindingList)
     */
    ...
    /**
     * @return Key set for additionalPredicates
     */
    public Set<String> keySetConst() {
	return additionalPredicates.keySet();
    }

    // if it's equality predicate add it to the bindings(I need to know about
    // the constant)
    /**
	...
	if (p.getName().equals(MediatorConstants.EQUALS)) {
		...
		// make sure that the constants are equal; otherwise unification
		// fails
		String s1 = t1.getSqlValNoType();
		String s2 = t2.getSqlValNoType();
		boolean unify = s1.equals(s2);
		if (unify == false) {
		    System.err.println("Unification failed for:(" + s1 + "="
			    + s2 + ")");
		}
		return unify;
	    } else {
