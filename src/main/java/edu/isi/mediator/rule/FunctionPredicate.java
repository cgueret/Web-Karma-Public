...
public class FunctionPredicate extends RelationPredicate {
    ...
    // for predicates that are functions there is only one out var = the only
    // free attribute
    /**
     ...
     * 
     * @return true if the output is a number false otherwise
     */
    public boolean outputIsNumber() {
	int i = source.getFirstFreeAttrPosition();
	// System.out.println("Func Source=" + source + " Attr=" +
	// source.getAttr(i) + " " + source.getAttr(i).isNumber());
	return source.getAttr(i).isNumber();
    }

    /**
     * Evaluate the function. All function definitions are in
     * edu.isi.mediator.gav.rule.FunctionsRepository.
     * <p>
     * Functions can have either a fixed number of input parameters, OR one
     * variable length input parameter.
     * <p>
     * For evaluation all terms should be ConstTerms, so that a value is
     * present.
     * 
     * @return
     * @throws MediatorException
	...
	try {
		...
		for (int i = 0; i < paramSize; i++) {
		    // when we evaluate the function all terms should be
		    // ConstTerms as we need values to evaluate it
		    Term t = terms.get(i);
		    if (!(t instanceof ConstTerm)) {
			throw new MediatorException(
				"At function evaluation all terms should be ConstTerm "
					+ this);
		    }
		...
		// so when we set it up we have to set it up as
		// "one array input"
	    ...
	    e.printStackTrace();
	    throw new MediatorException("Error during function invocation "
		    + this + ";" + e.getMessage());
	}
    }
     ...
     * <p>
     * Replaces all VarTerms with ConstTerms constructed with values from the
     * <p>
     * "values" Map.
     * 
     * @param values
     *            a value map where key=varName & value = varValue; <br>
     *            should contain values for the variables referred to in this
     *            function
     * @return
     * @throws MediatorException
     */
