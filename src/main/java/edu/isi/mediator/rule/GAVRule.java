...
public class GAVRule extends Rule {
    ...
    // for UAC
    /**
     * variable used in user access rule to denote ALL variables are visible in
     * that predicate
     */
    static public String ALL_VAR = "_all_";
    // it denotes that the predicate is not visible at all, can't even be used
    // in an internal join
    /**
     * variable used in user access rule to denote NO variables are visible in
     * that predicate
     */
    static public String NONE_VAR = "_none_";
    //
    // it denotes that the predicate is not visible at all, can be used in an
    // internal join
    /**
     * variable used in user access rule to denote NO variables are visible in
     * that predicate, <br>
     * but predicate can be used in a join
     */
    ...
    public boolean isNoneRule() {
	return false;
    }

    public boolean isUACRule() {
	return false;
    }

    public boolean isUACVar(String v) {
	if (v.equals(ALL_VAR) || v.equals(NONE_VAR)
		|| v.equals(NONE_BUT_JOINABLE_VAR))
	    return true;
	return false;
    }
    ...
    /**
     * Returns all equality relations. (BuiltInPredicate that represents
     * equality)
     * 
     * @return
     */
    ...
    /**
     * Returns all non-equality relations. (BuiltInPredicate that are NOT
     * equality)
     * 
     * @return
     */
	...
	for (int i = 0; i < antecedent.size(); i++) {
	    Predicate p = antecedent.get(i);
	    if ((p instanceof BuiltInPredicate)
		    && !p.getName().equals(MediatorConstants.EQUALS))
		rels.add(p);
	}
	return rels;
    }
    ...
    /**
     * Return the names of duplicate predicates. <br>
     * Predicates that appear more than once in the body.
     * 
     * @return
     */
     ...
     * @return true if the rule contains a predicate with the name pred.name
     * 
     */
     ...
     * 
     * @param name
     * @return all predicates with name equal to "name".
     */
     ...
     * 
     * @return true if predicate is valid, false otherwise. <br>
     *         make sure that all variables in the head are present in the body <br>
     *         make sure that all variables in a built-in predicate (x=3) appear
     *         in a body relation or the head
	...
	for (int j = 0; j < headVars.size(); j++) {
		...
		throw (new MediatorException("The head variable " + headVar
			+ " does not appear in the body of rule " + this));
	    }
	}

	// make sure that all variables in a built-in predicate (x=3) appear in
	// a relation in the body or head
	...
	for (int i = 0; i < nonRels.size(); i++) {
		...
		// System.out.println("Check var : " + pVar + " in " +
		// bodyRelationVars + " and " + headVars);
		if (!bodyRelationVars.contains(pVar)
			&& !headVars.contains(pVar))
		    throw (new MediatorException(
			    "The variable "
				    + pVar
				    + " does not appear in a body relation or head of rule "
				    + this));
		else {
		    if (!bodyRelationVars.contains(pVar)) {
			// it is in the head, but not the body => in the graph
			// it will be represented as an AssignNode
