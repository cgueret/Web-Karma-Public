...
/**
 * A Predicate can be a "normal" relation, a built-in predicate, like equality,
 * lessThan, like, etc., or a function. <br>
 * For a "normal" relation, the "name" is the name of the relation. <br>
 * {@link edu.isi.mediator.gav.rule.RelationPredicate} <br>
 * For built-in predicates like equality, lessThan, the "name" will be the
 * relation type <br>
 * {@link edu.isi.mediator.gav.rule.BuiltInPredicate}
 * 
 * @author mariam
 * 
 */
public abstract class Predicate {
    ...
    /**
     * true if domain predicate <br>
     * false if source predicate
     */
    ...
    public boolean isUACPredicate() {
	return isUACPredicate;
    }

    public void isUACPredicate(boolean b) {
	isUACPredicate = b;
    }
    ...
    /**
     * @return true if domain predicate <br>
     *         false if source predicate
     */
     ...
     * 
     * @param var
     *            variable name for {@link edu.isi.mediator.gav.domain.VarTerm} <br>
     *            constant value for
     *            {@link edu.isi.mediator.gav.domain.ConstTerm}
     */
    ...
    /**
     * Adds a term to the predicate if the term is not a duplicate of an
     * existing term.
     * 
     * @param var
     *            variable name for {@link edu.isi.mediator.gav.domain.VarTerm} <br>
    ...
    /**
     * Return the position of the first term,that is equal to t: starting at
     * "index"
     * 
     * @param t
     *            the term
	...
	// System.out.println("Find term " + t + " in " + terms +
	// " starting at " + index);
     ...
     * @param t1
     *            a term
     * @return true if the predicate contains a term equal to the specified term
     *         false otherwise
     */
     ...
     * 
     * @param i
     *            the position of the term to be removed
     */
     ...
     *         for a {@link edu.isi.mediator.gav.domain.VarTerm} the value is
     *         the variable name(var) <br>
     *         for a {@link edu.isi.mediator.gav.domain.ConstTerm} the value is
     *         the constant(val)
     */
     ...
     * 
     * @return the name of ALL variables <br>
     *         for the {@link edu.isi.mediator.gav.domain.ConstTerm} that do not
     *         have a given variable name a name is generated
     */
     ...
     * @param p
     *            a predicate
     * @return true if the predicates are equals <br>
     *         false otherwise <br>
     *         Two predicates are equal if the name is equals and all terms are
     *         equals
     */
    ...
    /**
     * Normalize a predicate with a given function. <br>
     * If the predicate contains "outVar" replace that Term with a FunctionTerm
     * whose function is set to "func"
     * 
     * @param func
     *            the function
     * @param outVar
     ...
     */
    public void normalizeWithFunction(FunctionPredicate func, String outVar)
	    throws MediatorException {
	// System.out.println("Normalize " + this + " with " + func + " outVar="
	// + outVar);
	for (int i = 0; i < terms.size(); i++) {
	    Term t = terms.get(i);
	    String var = t.getVar();
