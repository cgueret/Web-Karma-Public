...
public class RelationPredicate extends Predicate {
    ...
    /**
     * Returns an SQL compliant predicate name (escapes illegar chars in
     * predicate name)
     * 
     * @return an SQL compliant predicate name (escapes illegar chars in
     *         predicate name)
     */
     ...
     * 
     * @return type of this predicate; one of: <br>
     *         domain_predicate <br>
     *         function <br>
     *         ogsadqp
     */
     ...
     * 
     * @return name of all variables that need to be bound
     */
     ...
     * 
     * @return true if the predicate contains variables that need to be bound. <br>
     *         false otherwise
     */
     ...
     * @param attr
     *            the attribute name
     * @return "@" if the specified attribute is bound in the data source
     */
     ...
     * @param p
     *            the relation
     * @return attribute names that are common between the 2 relations
     */
     ...
     * @return the corresponding SourceAttr for the specified domainAttr
     */
	...
	if (source.getAttrs().size() != terms.size()) {
	    throw (new MediatorException("Number of attributes in predicate "
		    + this + " has to be equal with attrs in "
		    + source.getAttrs()));
	}
    ...
    /**
     * Returns the expanded conjunctive formula for this relation. <br>
     * Example: for a relation of the form: (x,y,x,"10") <br>
     * the formula: (x0,y1,x2,const3) ^ (const3="10") ^ (x0=x2) is returned.
     * 
     * @return the expanded conjunctive formula
     */
	...
	// System.out.println("Position of attrs=" + pos + " Constants=  " +
	// constants);
	...
	while (it.hasNext()) {
		...
		for (int i = 1; i < vals.size(); i++) {
		    BuiltInPredicate p1 = new BuiltInPredicate(
		    ...
		    v1.changeVarName(Integer.valueOf(vals.get(i - 1))
			    .intValue());
		    p1.addTerm(v1);
