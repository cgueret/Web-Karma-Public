...
/**
 * @author mariam Defines a BuiltInPredicate (equality, lessThan, like, etc.)
 *         Example: (X >= 5) (Y like 'foo')
 */
public class BuiltInPredicate extends Predicate {

    /**
     * true if this equality predicate is represented as an Assign operator in
     * the query graph. false otherwise (if it is represented as a Select
     * operator)
     */
    ...
    /**
     * @param name
     *            is one of: MediatorConstants.EQUALS
     *            MediatorConstants.GREATER_THAN
     *            MediatorConstants.GREATER_THAN_EQ MediatorConstants.LESS_THAN
     *            MediatorConstants.LESS_THAN_EQ MediatorConstants.LIKE
    ...
    /**
     * @return the value of isAssignment
     */
     ...
     * 
     * @return true if equality with same constant false otherwise
     */
    public boolean isEqualityWithSameConstant() {
	// a=a or NULL IS NULL
	if (name.equals(MediatorConstants.EQUALS)
		|| name.equals(MediatorConstants.IS_NULL)) {
	    Term t1 = terms.get(0);
	    Term t2 = terms.get(1);
