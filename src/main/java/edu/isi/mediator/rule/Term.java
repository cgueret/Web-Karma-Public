...
public abstract class Term {
    ...
    /**
     * name for select attributes used in final sql query (used only with SQL
     * interface; (equivalent to the "as" name ... select a as x)
     */
    protected String queryName;

    /**
     * true if this term can be seen after we apply uac rules false otherwise;
     * if it's false this term will become ConstTerm with value = "NOT ALLOWED"
     */
     ...
     * 
     * @param t
     * @return true if this term is equal to t
     */
    public abstract boolean equals(Term t);

    /**
     * Returns true if the source attribute associated with this term needs
     * binding.
     * 
     * @param b
     * @return
    ...
    /**
     * @param isNumber
     *            true if the type of this Term is a number false otherwise
     * @return value enclosed in '' if isNumber=false value with no quotes if
     *         isNumber=true
    ...
    /**
     * @return value as is; we don't know the type for this Term
     * @throws MediatorException
     */
     ...
     * 
     * @param b
     *            true if this term can be seen after we apply uac rules false
     *            otherwise; if it's false this term will become ConstTerm with
     *            value = "NOT ALLOWED"
     */
	...
	if (isAllowedAfterUAC == Visible.NOT_SET
		|| isAllowedAfterUAC == Visible.FALSE)
	    isAllowedAfterUAC = v;
	...
	// if not allowed from a _none_ concept I will see only vals from the
	// visible concept
	// if from _none_but_joinable_ it will join, so I will see only the
	// joined values, so OK
	// same if I have partial values visible, there will be a join
    }

    /**
     * @return true if isAllowedAfterUAC==Visible.NOT_SET ||
     *         isAllowedAfterUAC==Visible.TRUE false otherwise
     */
    public boolean isAllowed() {
	if (isAllowedAfterUAC == Visible.NOT_SET
		|| isAllowedAfterUAC == Visible.TRUE)
	    return true;
	else
	    return false;
    }
    ...
    /**
     * @return var for VarTerm and Functionterm val for ConstTerm
     */
    ...
    /**
     * @return constant value for ConstTerm null otherwise
     */
     ...
     * 
     * @return the variable name if this variable is not attached to a constant
     *         null otherwise
     */
