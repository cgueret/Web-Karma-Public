...
public class ConstTerm extends Term {
    ...
    /**
     * Constructs a ConstTerm with the variable "var" and value "val".
     * Associates this term with the SQL query name "queryName" (equivalent to
     * the "as" name ... select a as x)
     * 
     * @param var
     *            variable name
     * @param val
     *            constant value
     * @param queryName
     *            sql query variable name (used only with SQL interface;
     *            (equivalent to the "as" name ... select a as x)
     */
     ...
     * (non-Javadoc)
     * 
     * @see edu.isi.mediator.gav.domain.Term#getFreeVar() return null
     */
     ...
     * (non-Javadoc)
     * 
     * @see edu.isi.mediator.gav.domain.Term#getTermValue() return val
     */
     ...
     * 
     * @see
     * edu.isi.mediator.gav.domain.Term#equals(edu.isi.mediator.gav.domain.Term)
     */
     ...
     * 
     * @param t
     * @return true if the VALUE of the two terms is equal false otherwise
     */
     ...
     * (non-Javadoc)
     * 
     * @see edu.isi.mediator.gav.domain.Term#needsBinding(boolean) Always
     * returns false; A ConstTerm is already bound.
     */
     ...
     * 
     * @see
     * edu.isi.mediator.gav.domain.Term#unify(edu.isi.mediator.gav.domain.Binding
     * )
     */
     ...
     * 
     * @return the value as a number (not enclosed between single quotes)
	...
	try {
	    ...
	    /*
	     * else if(newV.startsWith("'")){ throw new MediatorException(newV +
	     * " should be a Number! Do not enclose it between \"'\""); }
	     */
	    ...
	    throw new MediatorException(newV + " should be a number!"
		    + ne.getMessage());
	} catch (Exception e) {
     ...
     * 
     * @return the value as a string (enclosed between single quotes)
     * @throws MediatorException
     */
	...
	if (newV.startsWith("\"") || newV.startsWith("'")) {
	    ...
	    throw new MediatorException(newV
		    + " should be a String! Enclose it between \"'\"");
	}
    }
     ...
     * @param val
     *            input value
     * @return NULL for an input of null (always uppercase) input value
     *         otherwise
     */
