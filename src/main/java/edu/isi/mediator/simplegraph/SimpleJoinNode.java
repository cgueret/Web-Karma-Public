...
public class SimpleJoinNode extends SimpleBaseNode {
    ...
    public SimpleJoinNode(ArrayList<String> joinAttrs, SimpleBaseNode child1,
	    SimpleBaseNode child2) {
	...
	for (int i = 0; i < joinAttributes.size(); i++) {
	    ...
	    // System.out.println("Add Join=" + bindMarker1 + actualName1 + "="
	    // + bindMarker2 + actualName2);
	    sqlWhere.add(bindMarker1 + actualName1 + "=" + bindMarker2
		    + actualName2);
	}
	...
	ArrayList<String> selectWithoutJoinAttrs = child1
		.removeFromSelect(joinAttributes);
