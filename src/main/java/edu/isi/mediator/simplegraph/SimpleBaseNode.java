...
public abstract class SimpleBaseNode {
	...
	sql += "select "
		+ distinctToken
		+ " "
		+ sqlSelect.toString().substring(1,
			sqlSelect.toString().length() - 1)
		+ " from "
		+ sqlFrom.toString().substring(1,
			sqlFrom.toString().length() - 1) + " ";
	if (sqlWhere.size() > 0)
	    sql += " where ";
	...
	for (int i = 0; i < sqlSelect.size(); i++) {
	    ...
	    // I want to make sure that I match with EXACTLY that attr name
	    // (that's why I have the " " at the end
	    int ind = oneSelect.indexOf(" as " + attr + " ");
	    if (ind > 0)
		return oneSelect.substring(0, ind);
	}
	// attr not found
	throw new MediatorException("Attribute " + attr + " not found in "
		+ sqlSelect);
    }

    // look in the select array, and find the actual name of all the vars inside
    // this predicate
    // it will be tableName.attrName as attr
    ...
    /**
     * Gets all source attributes names present in the sqlSelect list. <br>
     * These are all the selected attributes at this node.
     * 
     * @return all source attributes names present in the sqlSelect list.
     * @throws MediatorException
     */
    ...
    protected ArrayList<String> removeFromSelect(
