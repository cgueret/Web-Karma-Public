...
public class SimpleDataAccessNode extends SimpleBaseNode {
	...
	if (dn instanceof SimpleDataAccessNode) {
	    ...
	    ArrayList<String> joinAttrs = source
		    .findCommonAttrs(((SimpleDataAccessNode) dn).source);
	...
	// we use endsWith instead of startsWith because for sources that
	// include the OGSA-DAI resource
	// it looks like: TestR2_`table name`
	if (sourceName.endsWith("`"))
	    newName = sourceName.substring(0, sourceName.length() - 1) + "_"
		    + sourceId + "`";
	...
	for (int i = 0; i < source.getTerms().size(); i++) {
	    ...
	    select.add(tableName + "." + sourceAttr.getSQLName() + " as "
		    + t.getVar() + " ");
	}
	return select;
    }
    ...
    // if I have a bound attr that is not a constant it will be handled in the
    // Join
    /**
     * Returns the "where" SQL statement based on terms and sourceAttrs. <br>
     * If the term is a constant or it is a bound variable that is satisfied by
     * a constant <br>
	...
	for (int i = 0; i < source.getTerms().size(); i++) {
		...
		if (val.equals(MediatorConstants.NULL_VALUE))
		    where.add(bindMarker + tableName + "."
			    + sourceAttr.getSQLName() + " IS " + val);
		else
		    where.add(bindMarker + tableName + "."
			    + sourceAttr.getSQLName() + " = " + val);
	    }
	    if (t instanceof FunctionTerm) {
		if (child != null) {
		    // I have attribute binding patterns in the function
		    where.add(bindMarker
			    + tableName
			    + "."
			    + sourceAttr.getSQLName()
			    + " = "
			    + child.getActualName(((FunctionTerm) t)
				    .getFunction()));
		} else {
	...
	for (int i = 0; i < source.getTerms().size(); i++) {
		...
		while (j > 0) {
		    ...
		    if (j > 0) {
			SourceAttribute sourceAttr2 = source
				.getSourceAttribute(j);
			where.add(tableName + "." + sourceAttr1.getSQLName()
				+ " = " + tableName + "."
