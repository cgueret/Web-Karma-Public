...
public class SchemaParser {
     ...
     */
    public void parseSourceSchema(CommonTree schema,
	    ArrayList<String> functions, DomainModel dm)
	    throws MediatorException {
     ...
     * @throws MediatorException
     */
    private SourceSchema buildSourceSchema(CommonTree source,
	    ArrayList<String> functions) throws MediatorException {
     ...
     * @throws MediatorException
     */
    private SourceAttribute buildSourceAttribute(CommonTree attr)
	    throws MediatorException {
	String name = attr.getText();
	// remove ` if it exists
	// if I have an attribute called domain_schema I have to write
	// 'domain_schema` in the model
	...
	if (attr.getChildCount() != 2)
	    throw (new MediatorException(
		    "Attribute definition for SourceSchema is incorrect "
			    + attr.toStringTree()));
	String type = attr.getChild(0).getText();
	String binding = attr.getChild(1).getText();

	// System.out.println("name=" + name + " type=" + type + " binding=" +
	// binding);

	if (!binding.toUpperCase().equals(MediatorConstants.BOUND)
		&& !binding.toUpperCase().equals(MediatorConstants.FREE))
	    throw (new MediatorException("Binding can be only b|f "
		    + attr.toStringTree()));

	SourceAttribute sa = new SourceAttribute(name, type.toUpperCase(),
		binding.toUpperCase());
	return sa;
    }
     ...
     * @throws MediatorException
     */
    public void parseDomainSchema(CommonTree schema, DomainModel dm)
	    throws MediatorException {
	for (int i = 0; i < schema.getChildCount(); i++) {
     ...
     * @throws MediatorException
     */
    private DomainSchema buildDomainSchema(CommonTree domainSch)
	    throws MediatorException {
     ...
     * @throws MediatorException
     */
    private DomainAttribute buildDomainAttribute(CommonTree attr)
	    throws MediatorException {
	String name = attr.getText();
	// remove ` if it exists
	// if I have an attribute called domain_schema I have to write
	...
	if (attr.getChildCount() != 1)
	    throw (new MediatorException(
		    "Attribute definition for DomainSchema is incorrect "
			    + attr.toStringTree()));
	String type = attr.getChild(0).getText();
     ...
     * @throws MediatorException
     */
    public ArrayList<String> parseFunctions(CommonTree functions)
	    throws MediatorException {
