...
public class DomainParser {
    ...
    private static final MediatorLogger logger = MediatorLogger
	    .getLogger(DomainParser.class.getName());
	...
	for (int i = 0; i < t.getChildCount(); i++) {
		...
		ArrayList<GLAVRule> glavRules = glavRuleParser
			.parseGLAVRules(child);
		dm.setGLAVRules(glavRules);
	    } else if (child.getText().equals(MediatorConstants.LAV_RULES)) {
		ArrayList<LAVRule> lavRules = lavRuleParser
			.parseLAVRules(child);
	...
	// converts the input string to uppercase; so we can have case
	// insensitive parsing
	// in grammar only use uppercase constant values
	DomainModelLexer lex = new DomainModelLexer(
		new ANTLRNoCaseStringStream(domainStr));
	// passes the input string as is
	// DomainModelLexer lex = new DomainModelLexer(new
	// ANTLRStringStream(domainStr));
	...
	try {
	    ...
	    throw new MediatorException(ParserRecognitionException.getInstance(
		    e).toString());
	}
