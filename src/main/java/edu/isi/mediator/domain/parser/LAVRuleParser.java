...
public class LAVRuleParser {
     ...
     *            <p>
     *            For Example: (NAMESPACES (s 'http://www.domain.org/source/')
     *            (dv 'http://www.domain.org/ontology/'))
     */
     ...
     */
    public ArrayList<LAVRule> parseLAVRules(CommonTree rules)
	    throws MediatorException {
	...
	if (t.getChild(0).getChildCount() != 1) {
	    throw new MediatorException("There should be only one query in:"
		    + t);
	}
	...
	ArrayList<Predicate> antecedent = rp.parseConjunction(antecedentTree,
		null);
	ArrayList<Predicate> consequent = rp.parseConjunction(consequentTree,
		null);
