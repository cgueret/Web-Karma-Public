...
public class RuleParser {
     ...
     */
    public ArrayList<Predicate> parseConjunction(CommonTree rule, DomainModel dm)
	    throws MediatorException {
	...
	for (int i = 0; i < rule.getChildCount(); i++) {
		...
		predicates.add(p);
	    } else if (predicate.getText().equals(
		    MediatorConstants.BUILTIN_PRED)) {
     ...
     * @throws MediatorException
     */
    public RelationPredicate parsePredicateRelation(CommonTree predicate,
	    int predIndex) throws MediatorException {
	...
	for (int j = 1; j < predicate.getChildCount(); j++) {
		...
		FunctionPredicate fp = parseFunctionPredicate((CommonTree) predicate
			.getChild(j));
		...
		if (var.equals("_")) {
		    ...
		    // System.out.println("Var==" + var + " predI=" + predIndex
		    // + "id=" + id);
		}

		// why do I remove the backtick for the variables? If they have
		// a backtick I must
		// need them mariam:01/24/2012
		// at least I should record that it had a backtick
		// I especially need this for the RDF generation; If I loose the
		// info when I generate
     ...
     * @param predicate
     *            predicate as AST
     * @return FunctionPredicate Example: (FUNCTION_PRED f1 b (FUNCTION_PRED f2
     *         a)))
     * @throws MediatorException
     */
    private FunctionPredicate parseFunctionPredicate(CommonTree predicate)
	    throws MediatorException {
    ...
    // OLD implementation
    /*
     * private RelationPredicate parsePredicateRelation(CommonTree predicate,
     * int predIndex){
     * 
     * int id =0; //the name of the predicate String name =
     * predicate.getChild(0).getText(); RelationPredicate p = new
     * RelationPredicate(name);
     * 
     * for(int j=1; j<predicate.getChildCount(); j++){ String var =
     * predicate.getChild(j).getText();
     * 
     * if(var.equals("_")){ //give it a unique name var +=
     * String.valueOf(predIndex) + (id++); //System.out.println("Var==" + var +
     * " predI=" + predIndex + "id=" + id); } p.addTerm(var); } return p; }
     */
    /**
     ...
     */
    public BuiltInPredicate parsePredicateBuiltIn(CommonTree predicate)
	    throws MediatorException {
	...
	if (name.equals(MediatorConstants.IN)
		|| name.equals(MediatorConstants.NOT_IN)) {
     ...
     * @return BuiltInPredicate
     */
    public BuiltInPredicate parseINPredicate(String name, CommonTree predicate)
	    throws MediatorException {
     ...
     * @return operator required in sql query
     * @throws MediatorException
     */
