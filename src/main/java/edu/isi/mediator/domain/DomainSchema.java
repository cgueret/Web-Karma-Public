...
public class DomainSchema {
    ...
    // if name (the name of the table) has illegal chars (spaces for ex) i will
    // have to ecape this in the SQL `table name`
    private boolean hasIllegalChars = false;

    public DomainSchema(String name) {
	this.name = name;
	/*
	 * if(name.contains(ILLEGAR_CHARS)) hasIllegalChars=true;
	 */
    }
     ...
     * @param name
     *            the attribute name
     * @return true if the domain schema contains the specified attribute
     */
