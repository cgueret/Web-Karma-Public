...
/**
 * Class that provides methods for generating RDF from a database. Takes a
 * source description (rule) and an output file and generates triples. Outputs
 * the RDF triples in N3 notation in the output file or Stdout.
 * 
...
public class RuleRDFGeneratorDB extends RuleRDFGenerator {
    ...
    private static final MediatorLogger logger = MediatorLogger
	    .getLogger(RuleRDFGeneratorDB.class.getName());
	...
	super(r.rule, r.sourceNamespaces, r.ontologyNamespaces, r.outWriter,
		r.uniqueId);
    }
     ...
     * <p>
     * Uses the database defined by the connection string and retrieves tuples <br>
     * from the table defined by the rule name. Each tuple contains values <br>
     * for the attributes used in the rule. <br>
     * For each row in the table calls generateTriples(Map<String,String>
     * values).
     * 
     * @param conn
     *            database connection
     ...
     */
    public void generateTriplesDB(Connection conn) throws MediatorException,
	...
	// if table has a backtick in the domain file leave it like that in the
	// query
	...
	if (r == null) {
	    s.close();
	    throw new MediatorException("Could not execute query:"
		    + "select * from " + query);
	}
     ...
     * 
     * @return a SQL query from the given rule. <br>
     *         If the rule contains one predicate, returns a query of the form
     *         select * from predName. <br>
     *         If the rule contains a conjunction, the SQL query is a join
