...
/**
 * Class that provides methods for generating RDF. Takes a source description
 * (rule) and an output file and generates triples. Outputs the RDF triples in
 * N3 notation in the output file or Stdout.
 * 
...
public class RuleRDFGenerator {
    ...
    /**
     * if present in front of a value that represents a URI in the source that
     * URI will be used in the output RDF (instead of building a new URI)
     */
    ...
    /**
     * Contains mapping of column names to class names. key = column name; value
     * = class name key = NATIONALITY_ID; value = Country
     */
    ...
    /**
     * Used in conjunction with uniqueId to generate unique values. Each
     * processed tuple has a unique rowId. This rowId is added to the int in
     * uri(1), uri(2) making sure that the uri generated for uri(1), uri(2) are
     * unique, but that every time we see uri(1) the same value is generated.
     */
    protected int rowId = 0;

    /**
     * Stop RDF generation after MAX_NUMBER_OF_TRIPLES if = -1; write ALL
     * triples.
     */
    ...
    /**
     * Constructs a RuleRDFGenerator. <br>
     * NOTE: DOES NOT write namespaces to the output stream.
     * 
     * @param rule
     *            source description; can be either LAV or GLAV rule
     *            <p>
     *            For ex: DEF_Nationality(NATIONALITY_ID, NATIONALITY) ->
     *            Country(uri(NATIONALITY_ID)) ^ hasName(uri(NATIONALITY_ID),
     *            NATIONALITY)
     * @param sourceNamespaces
     *            map containing (prefix,namespace)
     *            <p>
     *            For ex: s:http://www.host.com/SOURCE
     * @param ontologyNamespaces
     *            map containing (prefix,namespace)
     *            <p>
     *            For ex: o:http://www.host.com/ONTOLOGY
     * @param outWriter
     *            output writer
     * @throws MediatorException
     * @throws IOException
     */
    public RuleRDFGenerator(Rule rule, Map<String, String> sourceNamespaces,
	    Map<String, String> ontologyNamespaces, PrintWriter outWriter)
	...
	// gets populated only if sourceNamespaces & ontologyNamespaces contain
	// only one entry
    ...
    public RuleRDFGenerator(Rule rule, Map<String, String> sourceNamespaces,
	    Map<String, String> ontologyNamespaces, PrintWriter outWriter,
	    String uniqueId) throws MediatorException {
    ...
    /**
     * Sets the maximum number of generated triples. Process is aborted after
     * the MAX_NUMBER_OF_TRIPLES is reached. <br>
     * Use -1 to generate ALL triples.
     * 
     ...
     * 
     * @return the number of generated triples.
     */
     ...
     * @return true if MAX_NUMBER_OF_TRIPLES was not reached, false otherwise.
     * @throws MediatorException
     * @throws UnsupportedEncodingException
     */
    public boolean generateTriples(Map<String, String> values)
	...
	/*
	 * only for printing... fails if getValue=null String valuesStr = "";
	 * Set<Entry<String,String>> entries = values.entrySet();
	 * Iterator<Entry<String,String>> it = entries.iterator();
	 * while(it.hasNext()){ Entry<String,String> entry = it.next();
	 * valuesStr += entry.getKey() + ":" +
    ...
    /**
     * Generates triples for unary predicates of type: <br>
     * ClassName(uri(ColumnName))
     * 
     * @param values
     *            map containing <column_name,value> where "column_name" are
     *            variable names used in the rule.
     * @return true if MAX_NUMBER_OF_TRIPLES was not reached, false otherwise.
     * @throws MediatorException
     *             <p>
     *             Example: <br>
     *             Generates triples of the form:X a Y; <br>
     *             s:Country_10 a dv:Country ;
     * @throws UnsupportedEncodingException
     */
    protected boolean addClassStatements(Map<String, String> values)
	    throws MediatorException, UnsupportedEncodingException {
     ...
     *             <p>
     *             Example: <br>
     *             Generates a triple of the form:X Y Z; <br>
     *             s:Person_78 dv:hasNationality s:Country_10 .
     * @throws UnsupportedEncodingException
     */
    protected boolean addPredicateStatements(Map<String, String> values)
	    throws MediatorException, UnsupportedEncodingException {
     ...
     *             <p>
     *             Example: <br>
     *             Generates a triple of the form:X a Y; <br>
     *             s:Country_10 a dv:Country ;
     * @throws UnsupportedEncodingException
     */
	...
	if (!(t instanceof FunctionTerm))
	    throw new MediatorException(
		    "A subject should have only one uri FunctionTerm: " + p);

	FunctionPredicate uri = ((FunctionTerm) t).getFunction();

	String subject = evaluateURI(uri, className, values);

	// we assume that class names do not have spaces or other "strange"
	// chars
    ...
    /**
     * Generates triple for a binary predicate of type: <br>
     * PredicateName(uri(ColumnName),Value) or <br>
     * PredicateName(uri(ColumnName1),uri(ColumnName2))
     * 
     * @param p
     *            the predicate
     ...
     *             Example: <br>
     *             Generates a triple of the form:X Y Z; <br>
     *             dv:hasNationality s:Person_78 , s:Country_10 .
     * @throws UnsupportedEncodingException
     */
    protected boolean addPredicateStatement(Predicate p,
	    Map<String, String> values) throws MediatorException,
	...
	// we assume that predicate names do not have spaces or other "strange"
	// chars
	...
	if (!(t1 instanceof FunctionTerm))
	    throw new MediatorException(
		    "First term in predicate should be uri FunctionTerm: " + p);
	...
	if (t2 instanceof FunctionTerm) {
	    ...
	    if (varValue == null)
		throw new MediatorException(
			"The values map does not contain variable: " + varName
				+ " Map is:" + values);

	    statement += prepareValue(varValue);

	    // I have a key that has no value OR has NULL value=>don't add this
	    // triple
	    // all NULL values are transformed to "" before I get here, so that
	    // I can
     ...
     * 
     * @param value
     * @return a value valid for RDF N3 notation. <br>
     *         Note: look in RuleRDFMapper for a special case where the value is
     *         a URI.
     */
    ...
    /**
     * Builds a URI FunctionPredicate and evaluates it. Adds additional terms to
     * the existing predicate, namely: sourcePrefix, sourceNamespace,className
     * and isSeed Returns the URI constructed with the given values.
     * 
     * @param uri
     *            the uri() predicate
     * @param className
     *            the class that corresponds to the column names referred to in
     *            the uri function
     * @param values
     *            map with <column_name, value>
     * @return the URI constructed with the given values.
     * @throws MediatorException
     *             <p>
     *             A uri predicate can be: <br>
     *             uri(var_name) - in this case we get the associated value from
     *             <br>
     *             values and use this value to construct the URI and term
     *             "Seed" = false <br>
     *             uri(int) - in this case the integer specifies a seed used
     *             during the <br>
     *             evaluation of uri() to generate a random number and term
     *             "Seed" =true
     * @throws UnsupportedEncodingException
     */
    protected String evaluateURI(FunctionPredicate uri, String className,
	    Map<String, String> values) throws MediatorException,
	    UnsupportedEncodingException {
	...
	if (varName == null) {
		...
		if (val == null)
		    throw new MediatorException(
			    "The values map does not contain variable: "
				    + varName + " Map is:" + values);
		if (i > 0) {
		    varValue += "_";
		    allVarNames += "_";
		}
		if (val.equals("NULL")) {
		    ...
		    // in case I have more than 1 NULL in a row I have to
		    // distinguish
		    // between them, so I use the column id to generate unique
		    // gensyms
	...
	if (className == null) {
		...
		if (uri.getTerms().size() == 1 && varValue.startsWith(URI_FLAG)) {
		    // I have a value that is already a URI, so I don't need to
		    // construct it
		    // I can just use it as is
		} else {
		    throw new MediatorException(
			    "Did not find equivalent class for:" + allVarNames
				    + " in " + classes);
		}
	    }
	} else {
	...
	// add sourceUri
	ConstTerm sourceUriTerm = new ConstTerm("SourceUri",
    ...
    /**
     * Writes a triple to the output. <br>
     * Returns true if MAX_NUMBER_OF_TRIPLES was not reached, false otherwise.
     * 
     * @param statement
     * @return true if MAX_NUMBER_OF_TRIPLES was not reached, false otherwise.
     */
