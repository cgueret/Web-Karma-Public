...
/**
 * Class that provides methods for generating RDF aligned to a predefined
 * ontology using Karma source description rules. <br>
 * Generates triples in N3 notation. <br>
 * RDF generation stops after the MAX_NUMBER_OF_TRIPLES is reached.
 * 
...
public class OntologyMapper {
    ...
    /**
     * map that contains variable names and their corresponding object name. Ex:
     * if INCIDENT_ID belongs to table INCIDENT, key=INCIDENT_ID;val=?o_INCIDENT
     * val is the object corresponding to the URI that we will map INCIDENT_ID
     * to
     */
    Map<String, String> objectURI = new HashMap<String, String>();

    private static final MediatorLogger logger = MediatorLogger
	    .getLogger(OntologyMapper.class.getName());

    /**
     * Constructs an OntologyMapper given a RDF model. <br>
     * Maximum number of triples generated is 45000.
     * 
     * @param model
     *            the RDF model
     */
    ...
    /**
     * Constructs an OntologyMapper given a RDF model and the maximum number of
    ...
    /**
     * Returns RDF triples (in N3 notation) aligned to a predefined ontology
     * defined by <br>
     * Karma source description rules. <br>
     * RDF generation stops after the MAX_NUMBER_OF_TRIPLES is reached.
     * 
     * @param uri
     *            uri that needs to be aligned.
     * @return RDF triples in N3 notation.
     * @throws MediatorException
     * @throws IOException
     */
    public String getAlignedRdf(String uri) throws MediatorException,
	    IOException {
    ...
    /**
     * Generates RDF triples (in N3 notation) aligned to a predefined ontology
     * defined by <br>
     * Karma source description rules. Generated RDF is written to the file
     * "outputFileName" <br>
     * RDF generation stops after the MAX_NUMBER_OF_TRIPLES is reached.
     * 
     * @param uri
     ...
     * @throws IOException
     */
    public void getAlignedRdf(String uri, String outputFileName)
     ...
     * Karma source description rules. Generated RDF is written to the file
     * "outputFileName" <br>
     * RDF generation stops after the MAX_NUMBER_OF_TRIPLES is reached. <br>
     * if outStr is null and outFile !=null => write to a file <br>
     * if outStr != null and outFile =null => write to a string
     * 
     * @param uri
     *            uri that needs to be aligned.
     * @param outStr
    ...
    /**
     * Returns a PrintWriter. <br>
     * if outStr is null and outFile !=null => PrintWriter to a file <br>
     * if outStr != null and outFile =null => PrintWriter to a string
     * 
     * @param outStr
     *            the output string OR null if the output is written to a file
     * @param outFile
    ...
    /**
     * Executes each query generated by the queryRule and accumulates all
     * values. <br>
     * We could have more than one query if we have a relationship table t(var1,
     * var2) <br>
     * where D2R generates the triple "var1 tableName var2" (tableName is the
     * property) OR "var2 tableName var1"
     * 
     * @param rule
     *            the mapping rule
     * @param tableName
     ...
     * @return all values generated by executing SPARQL queries defined by
     *         queryRule.
     * @throws MediatorException
     */
    ...
    /**
     * Returns attr/value pairs for all tuples generated by executing a SPARQL
     * query. <br>
     * In the initial query the object that corresponds to "tableName" is
     * replaced by URI. <br>
     * URI refers to objects present in the table given by "tableName".
     * 
     * @param select
     ...
     * @param uri
     *            the input URI
     * @return attr/value pairs for all tuples generated by executing the query.
     * @throws MediatorException
     */
    ...
    /**
     * Returns the select part of a SPARQL query constructed from the queryRule. <br>
     * Adds to the select all variables and all objects referring to predicates. <br>
     * These objects contain intermediary URIs necessary when building the final
     * RDF. <br>
     * Example: Rule: r(a,b,c,d)<- s1(a,b,c)^s2(`c*`,d) ; c* represents a
     * primary key (used to make the joins in the query) SELECT DISTINCT ?a ?b
     * ?c ?d ?o_s1 ?o_s2
     * 
     * @param rule
     *            the mapping rule (LAV or GLAV).
     * @return the select part of a SPARQL query constructed from the queryRule.
     * @throws MediatorException
     */
     ...
     * Multiple queries could be returned if we have relationship predicates.
     * Look at getRelationshipQuery(). <br>
     * The query is generated by combining the predicates from the queryRule. <br>
     * The objects and property names are constructed from the table/column
     * names. <br>
     * Example: Rule: r(a,b,c,d)<- s1(a,b,c)^s2(`c*`,d) ; c* represents a
     * primary key (used to make the joins in the query) WHERE { ?o_s1
     * vocab:s1_a ?a . ?o_s1 vocab:s1_b ?b . ?o_s1 vocab:s1_c ?o_s2 . ?o_s2
     * vocab:s1_c ?c . ?o_s2 vocab:s1_d ?d . Rule: r(a,b,c,d)<-
     * s1(a,b,`c*`)^s2(c,d) ; and s2 is a relationship predicate 2 queries are
     * returned: WHERE { ?o_s1 vocab:s1_a ?a . ?o_s1 vocab:s1_b ?b . ?o_s1
     * vocab:s1_c ?o_s2 . ?o_s1 vocab:s2 ?d } and WHERE { ?o_s1 vocab:s1_a ?a .
     * ?o_s1 vocab:s1_b ?b . ?o_s1 vocab:s1_c ?o_s2 . ?d vocab:s2 ?o_s1 } and
     * 
     * @param rule
     *            the mapping rule (LAV or GLAV).
     * @return the where part of a SPARQL query constructed from the queryRule.
     * @throws MediatorException
     */
    ...
    /**
     * Returns a mapping between primary keys and the objects that they belong
     * to. key = attribute name; value = obj that this attr belongs to. n
     * attribute is a key if it ends with "*". Example: {c, o_s2)
     * 
     * @param rule
     *            the mapping rule (LAV or GLAV).
     * @return a mapping between primary keys and the objects that they belong
     *         to.
     * @throws MediatorException
     */
    private HashMap<String, String> getKeyObjects(Rule rule)
	    throws MediatorException {
	...
	for (int i = 0; i < rule.getAntecedent().size(); i++) {
		...
		if (var.endsWith("*")) {
		    ...
		    if (value != null) {
			throw new MediatorException("A primary key with name "
				+ newVar + " already exists.");
		    }
    ...
    /**
     * Returns SPARQL triples for a given predicate. <br>
     * Replaces foreign key objects with the objects from keyObjects. <br>
     * This generates the correct joins in the query. <br>
     * Example: s1(a,b,c) ?o_s1 vocab:s1_a ?a . ?o_s1 vocab:s1_b ?b . ?o_s1
     * vocab:s1_c ?o_s2 .
     * 
     * @param p
     *            the predicate
     ...
     * @return SPARQL triples for a given predicate.
     */
    ...
    /**
     * Returns two queries for the given predicate. <br>
     * Note: it is a relationship predicate with 2 variables. <br>
     * this is represented in RDF as (var1 prop:tableName var2) or (var2
     * prop:tableName var1) <br>
     * Example: s1(a,b) : if a or b are foreign keys we replace ?a or ?b with
     * the corresponfing object ?a vocab:db.s1 ?b . ?b vocab:db.s1 ?a .
     * 
     * @param p
     *            the predicate
     * @param keyObjects
    ...
    /**
     * Returns true if this is a relationship predicate. <br>
     * The correct algorithm to determine if it's a relationship predicate is to <br>
     * get all defined predicates and if we find the predicate
     * dbName.(p.getName()) <br>
     * then it's a relationship pred. I couldn't find a way to get all defined
     * predicates.
     * 
     * @param p
     * @return true if this is a relationship predicate, false otherwise.
     * @throws MediatorException
     */
    ...
    /**
     * Returns a mediator domain file containing the rules relevant to
     * tableName.
     * 
     * @param tableName
     * @return
     * @throws MediatorException
     */
    private String getSourceDescription(String tableName)
	    throws MediatorException {
	...
	if (allRules.isEmpty())
	    throw new MediatorException("No transformation rule found for: "
		    + tableName);
	...
	for (int i = 0; i < allRules.size(); i++) {
	    ...
	    List<Map<String, String>> allNamespaces = JenaUtil.executeQuery(
		    model, newQ);
