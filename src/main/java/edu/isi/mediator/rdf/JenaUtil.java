...
public class JenaUtil {
    ...
    private static final MediatorLogger logger = MediatorLogger
	    .getLogger(JenaUtil.class.getName());
     ...
     * @param prefixName
     *            prefix name
     * @return the prefix with the given name. Throws an exception if the prefix
     *         cannot be found.
     * @throws MediatorException
     */
    static public String getNamespace(Model model, String prefixName)
	    throws MediatorException {
	...
	if (prefix == null) {
	    throw new MediatorException("The prefix " + prefixName
		    + " cannot be found!");
	}
	return prefix;
    }
    ...
    /**
     * Returns the namespace of properties related to the input URI. <br>
     * Get the properties if the input URI, and extract the namespace.
     * 
     * @param model
     *            RDF model
     * @param uri
     ...
     * @return the namespace of properties related to the input URI.
     * @throws MediatorException
     */
    static public String getNamespace(Model model, String uri, String tableName)
	    throws MediatorException {
	String propertyPrefix = "";
	String q = "SELECT DISTINCT ?p WHERE {" + " <" + uri + "> ?p ?o . }";
	List<Map<String, String>> propertyValues = executeQueryExtractValues(
		model, q);
    ...
    /**
     * Returns attr/value pairs for all tuples generated by executing the query. <br>
     * For values that are URI, it marks the URI to distinguish between literals
     * that are a URI (start with http) and an actual resource URI(see
     * getOneRowMarkURI()).
     * 
     * @param model
     *            RDF model
     ...
     * @throws MediatorException
     */
    static public List<Map<String, String>> executeQueryExtractValues(
	    Model model, String q) throws MediatorException {
	...
	while (results.hasNext()) {
	    ...
	    // it is streaming the solutions, so as soon as we have a solution
	    // it returns it
     ...
     * @throws MediatorException
     */
    static public List<Map<String, String>> executeQuery(Model model, String q)
	    throws MediatorException {
	List<Map<String, String>> allRows = new ArrayList<Map<String, String>>();
	logger.debug("Execute SPARQL query ..." + q);
	// Execute the query and obtain results
     ...
     * @param valueAsURI
     *            true if the value is returned as is: URI or Literal false if
     *            we extract the value from the URI
     * @return attr/value pairs for one row of data.
     * @throws MediatorException
     */
    static private Map<String, String> getOneRow(QuerySolution qs,
	    List<String> vars, boolean valueAsURI) throws MediatorException {
    ...
    static private Map<String, String> getOneRowMarkURI(QuerySolution qs,
	    List<String> vars) throws MediatorException {
	Map<String, String> onerow = new HashMap<String, String>();
	for (int j = 0; j < vars.size(); j++) {
	    String var = vars.get(j);
	    RDFNode node = qs.get(var);
		...
		// I need to know this in the uri(), so I don't confuse a URI
    ...
    /**
     * Returns the value from a URI. <br>
     * The URI should contain DbName.TableName/Value <br>
     * Look for last "."; this is the beginning of the table name <br>
     * from there search for "/" (not sure what happens if the table name
     * contains /)
     * 
     * @param uri
     *            the URI.
     * @return the value from a URI.
     * @throws MediatorException
     */
	...
	if (ind1 < 0) {
	    throw new MediatorException(
		    "The subject URI must contain a table name that starts with \"DBNAME.\"");
	}
	int ind2 = uri.toString().indexOf("/", ind1);
	if (ind2 < 0) {
	    throw new MediatorException(
		    "The subject URI must contain a table name that starts with \".\" and ends with \"/\", and a value."
