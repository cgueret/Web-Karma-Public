...
/**
 * Class that provides methods for generating RDF. Takes a domain file
 * containing source descriptions (rules), a connect string to a database or a
 * CSV file and an output file and generates triples. Output is in N3 notation.
 * 
...
public class RDFGenerator {
    ...
    /**
     * Database connection string to the DB that provides values for the
     * attributes in the rules.
     */
    private Connection conn;

    /**
     * Unique id, used for generating value for attribs thathave no value in the
     * DB.
     */
     ...
     * 
     * @param domainFile
     *            The domain file name. Should contain "NAMESPACES" and
     *            "LAV_RULES" sections.
     * @param outputFile
     ...
     *            location of output file OR null if output to Stdout
     * @param b
     *            not used, can be any value (to distinguish between
     *            constructors) <br>
     *            I did not want to change the previous constructor for backward
     *            compatibility
     * @throws MediatorException
     * @throws IOException
     * @throws ClassNotFoundException
	...
	for (int i = 0; i < dm.getAllRules().size(); i++) {
	    ...
	    RuleRDFGenerator rgen = new RuleRDFGenerator(rule,
     ...
     * @throws MediatorException
     */
    private Connection getDbConnection(String connectStr, String dbDriver)
	    throws MediatorException {
	...
	try {
	    ...
	    throw new MediatorException("Error occured while connecting to:"
		    + connectStr + " " + e.getMessage());
	}
	return localConn;
    }
     ...
     * @throws SQLException
     */
    public void generateTriples(String input, String sourceType)
	    throws MediatorException, IOException, SQLException,
     ...
     */
    private void generateTriplesDB(String connectStr, String dbDriver)
	    throws MediatorException, SQLException, ClassNotFoundException,
	...
	for (int i = 0; i < rdfGenerators.size(); i++) {
	    RuleRDFGeneratorDB gen = new RuleRDFGeneratorDB(
		    rdfGenerators.get(i));
     ...
     * @throws IOException
     */
    private void generateTriplesCSV(String inputFile) throws MediatorException,
	    IOException {
	// only one rule should be present in the rule file (one input
	// file/rule)
	// we could have several rules for the same concept
	// if(rdfGenerators.size()!=1){
	// throw new
	// MediatorException("Source Descriptions file should contain ONE rule.");
	// }
	for (int i = 0; i < rdfGenerators.size(); i++) {
	    RuleRDFGeneratorCSV gen = new RuleRDFGeneratorCSV(
		    rdfGenerators.get(i));
     ...
     * @throws IOException
     */
    private void generateTriplesACCESS(String accessDb)
	    throws MediatorException, IOException {
	for (int i = 0; i < rdfGenerators.size(); i++) {
	    RuleRDFGeneratorAccess gen = new RuleRDFGeneratorAccess(
		    rdfGenerators.get(i));
