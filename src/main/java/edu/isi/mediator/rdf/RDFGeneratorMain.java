...
/**
 * Main RDF Generator Class. Reads a configuration file called
 * rdfsettings.config located in the top level directory.
 * 
...
public class RDFGeneratorMain {
    ...
    private static final MediatorLogger logger = MediatorLogger
	    .getLogger(RDFGeneratorMain.class.getName());
	...
	try {
		...
		if (ind <= 0)
		    throw new MediatorException(
			    "Settings should be of form: PropertyName=PropertyValue "
				    + line);
	...
	if (ruleFile == null) {
	    throw new MediatorException(
		    "Settings file is missiong SOURCE_DESC.");
	}
	if (inputsUsed == 0)
	    throw new MediatorException(
		    "Settings file should contain either INPUT_FILE, CONNECT_STR or ACCESS_DB");
	if (inputsUsed > 1)
	    throw new MediatorException(
		    "Settings file should contain only ONE of INPUT_FILE, CONNECT_STR or ACCESS_DB.");
	if (connectStr != null && dbDriver == null) {
	    throw new MediatorException(
		    "Settings file should contain DB_DRIVER.");
	}
    }
