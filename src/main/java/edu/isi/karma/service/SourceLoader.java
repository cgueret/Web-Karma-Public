...
public class SourceLoader {
	...
	String source_id = uri.substring(uri.lastIndexOf("/") + 1,
		uri.length() - 1);
	String dir = Repository.Instance().SOURCE_REPOSITORY_DIR;
	String fileName = source_id
     ...
     * 
     * @param limit
     *            : maximum number of results, null value means all the sources
     * @return
     */
	...
	if (sourceLimit != null) {
	    ...
	    queryString += "LIMIT "
		    + String.valueOf(sourceLimit.intValue() + "\n");
	}
	...
	try {
		...
		source_id = source_uri.substring(
			source_uri.lastIndexOf("/") + 1,
			source_uri.length() - 1);
    ...
    /**
     * returns all the sources in the repository along with their complete
     * information probably this method is not useful since fetching all the
     * sources with their complete information takes long time.
     * 
     * @return
     */
    ...
    /**
     * Searches the repository to find the sources whose model matches the
     ...
     *            repository
     * @param ioType
     *            declares which one of the source input or source output will
     *            be tested for matching
     * @param sourceLimit
     *            maximum number of sources that will be fetched
     * @return a hashmap of all found sources and a mapping from the found
     *         source parameters to the model parameters. This help us later to
     *         how to join the model's corresponding source and the matched
     *         source
     */
    public static Map<Source, Map<String, String>> getSourcesByIOPattern(
	    edu.isi.karma.service.Model semanticModel, Integer sourceLimit) {
	...
	Map<Source, Map<String, String>> sourcesAndMappings = new HashMap<Source, Map<String, String>>();

	Map<String, Map<String, String>> sourceIdsAndMappings = semanticModel
		.findInJenaModel(Repository.Instance().getModel(), sourceLimit);
	...
	for (String sourceId : sourceIdsAndMappings.keySet()) {
	    ...
	    if (m != null)
		sourcesAndMappings.put(getSourceFromJenaModel(m),
			sourceIdsAndMappings.get(sourceId));
	}
    ...
    /**
     * Searches the repository to find the sources whose model is contained in
     * the semantic model parameter. Note that the services in the return list
     * only include the operations that match the model parameter.
     * 
     * @param semanticModel
     *            The input model whose pattern will be searched in the
     *            repository
     ...
     *         service
     */
    public static Map<Source, Map<String, String>> getSourcesContainedInModel(
	    edu.isi.karma.service.Model semanticModel, Integer sourceLimit) {
	...
	for (Source source : sourceList) {
	    ...
	    Map<String, Map<String, String>> sourceIdsAndMappings = m
    ...
    /**
     * From the source model, returns the source object If the list of operation
     * ids is null, it includes all the operations, otherwise it only fetches
     * the mentioned operations If operationLimit=null and operationIds=null
     * returns all operations If operationLimit=null, it returns the all
     * specified operations If operationIds=null, it returns the first (opLimit)
     * number of operations If both of them are not null, it returns the first
     * (opLimit) number of specified operations
     * 
     * @param model
     * @param operationIds
     * @return
	...
	// source local id
	source_id = source_uri.substring(source_uri.lastIndexOf("/") + 1,
		source_uri.length() - 1);
	logger.debug("source id: " + source_id);
	...
	// source name
	nodeIterator = model.listObjectsOfProperty(source_resource,
		has_name_property);
    ...
    private static Attribute getSingleAttribute(Model model,
	    Resource att_resource) {
	...
	Attribute att = new Attribute(att_id, att_resource.getNameSpace(),
		att_name, IOType.NONE, AttributeRequirement.NONE);
	...
	// String uri =
	// "http://isi.edu/integration/karma/sources/940466A8-8733-47B1-2597-11DC112F0437F#";
	...
	URI featurePredicatName = new URI(geonamesOntology + "Feature",
		geonamesOntology, "gn");
	URI latPredicatName = new URI(wgs84Ontology + "lat", wgs84Ontology,
		"wgs84");
	URI lngPredicatName = new URI(wgs84Ontology + "long", wgs84Ontology,
		"wgs84");
	...
	Map<Source, Map<String, String>> sourcesAndMappings = getSourcesByIOPattern(
		semanticModel, null);
	...
	for (Source s : sourcesAndMappings.keySet()) {
		...
		System.out.println(str + "-------"
			+ sourcesAndMappings.get(s).get(str));
	}
