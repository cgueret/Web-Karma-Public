...
public class Model {
	...
	// Property has_model = model.createProperty(Namespaces.KARMA,
	// "hasModel");
	...
	for (Atom atom : this.getAtoms()) {
		...
		if (propertyAtom.getPropertyPredicate().getPrefix() != null
		    ...
		    model.setNsPrefix(propertyAtom.getPropertyPredicate()
			    .getPrefix(), propertyAtom.getPropertyPredicate()
			    .getNs());
		...
		Resource arg2 = model.getResource(this.baseUri
    ...
    public Map<String, Map<String, String>> findInServiceInputs(
	    com.hp.hpl.jena.rdf.model.Model serviceJenaModel,
	    Integer serviceLimit) {
	return findSemanticModelInJenaModel(serviceJenaModel, IOType.INPUT,
		serviceLimit);
    }
    ...
    public Map<String, Map<String, String>> findInServiceOutputs(
	    com.hp.hpl.jena.rdf.model.Model serviceJenaModel,
	    Integer serviceLimit) {
	return findSemanticModelInJenaModel(serviceJenaModel, IOType.OUTPUT,
		serviceLimit);
    }
    ...
    /**
     * Returns a hash map of all sources/services in the jena model whose
     * input/output matches this object model.
     * 
     * @param semanticModel
     ...
     * @return a hashmap of all IDs of found sources/services and a mapping from
     *         the model parameters to query params. This help us later to join
     *         the model's corresponding source and the matched service
     */
    private Map<String, Map<String, String>> findSemanticModelInJenaModel(
	    com.hp.hpl.jena.rdf.model.Model jenaModel, String ioType,
	    Integer limit) {

	Map<String, Map<String, String>> IdsAndMappings = new HashMap<String, Map<String, String>>();
	...
	try {
		...
		for (String arg : argList) {
			...
			String retrieved_id = argNode.asResource()
				.getLocalName();
	...
	List<Map<String, String>> attValueList = new ArrayList<Map<String, String>>();
	...
	try {
	    ...
	    for (; results.hasNext();) {
		QuerySolution soln = results.nextSolution();
		Map<String, String> attValues = new HashMap<String, String>();
    ...
    public void writeJenaModelToFile(String path, String lang)
	    throws FileNotFoundException {
	com.hp.hpl.jena.rdf.model.Model model = getJenaModel();
	String source_desc_file = path
		+ Repository.Instance().getFileExtension(lang);
	OutputStreamWriter output = new OutputStreamWriter(
		new FileOutputStream(source_desc_file));
	...
	for (int i = 0; i < this.getAtoms().size(); i++) {
		...
		if (atom instanceof ClassAtom) {
			...
			nsToPrefixMapping.put(classAtom.getClassPredicate()
				.getNs(), classAtom.getClassPredicate()
				.getPrefix());
			predicateUri = classAtom.getClassPredicate()
				.getLocalNameWithPrefix();

		    } else {
			predicateUri = "<"
				+ classAtom.getClassPredicate().getUriString()
				+ ">";
		    }
		    ...
		    select_where += "      " + argument1Var + " rdf:type "
			    + predicateUri + " . \n";
		} else if (atom instanceof PropertyAtom) {
		    PropertyAtom propertyAtom = ((PropertyAtom) atom);
			...
			nsToPrefixMapping.put(propertyAtom
				.getPropertyPredicate().getNs(), propertyAtom
				.getPropertyPredicate().getPrefix());
			predicateUri = propertyAtom.getPropertyPredicate()
				.getLocalNameWithPrefix();

		    } else {
			predicateUri = "<"
				+ propertyAtom.getPropertyPredicate()
					.getUriString() + ">";
		    }
	...
	for (int i = 0; i < this.getAtoms().size(); i++) {
		...
		if (atom instanceof ClassAtom) {
		    ...
		    construct_header += "      " + argument1Var + " rdf:type "
			    + predicateUri + " . \n";

		    construct_where += "      " + argument1Var + " rdf:type "
			    + predicateUri + " . \n";
		} else if (atom instanceof PropertyAtom) {
		    PropertyAtom propertyAtom = ((PropertyAtom) atom);
	...
	for (int i = 0; i < m.getAtoms().size(); i++) {
		...
		if (atom instanceof ClassAtom) {
		    ...
		    select_where += "      ?model " + Prefixes.KARMA
			    + ":hasAtom " + atomVar + " . \n" + "      "
			    + atomVar + " a " + Prefixes.SWRL
			    + ":ClassAtom . \n" + "      " + atomVar + " "
			    + Prefixes.SWRL + ":classPredicate <"
			    + predicateUri + "> . \n" + "      " + atomVar
			    + " " + Prefixes.SWRL + ":argument1 "
			    + argument1Var + " . \n";
		} else if (atom instanceof PropertyAtom) {
		    PropertyAtom propertyAtom = ((PropertyAtom) atom);
		    atomVar = "?atom" + String.valueOf(i + 1);
		    ...
		    select_where += "      ?model " + Prefixes.KARMA
			    ...
			    + atomVar + " " + Prefixes.SWRL
			    + ":propertyPredicate <" + predicateUri + "> . \n"
			    + "      " + atomVar + " " + Prefixes.SWRL
			    + ":argument1 " + argument1Var + " . \n" + "      "
			    + atomVar + " " + Prefixes.SWRL + ":argument2 "
			    + argument2Var + " . \n";
		}
	    }
	}
	...
	for (Atom atom : atoms) {
		...
		if (atom instanceof ClassAtom) {
		    ...
		    PropertyAtom propertyAtom = ((PropertyAtom) atom);
		    logicalForm += propertyAtom.getPropertyPredicate()
			    .getLocalName();
		    logicalForm += "(";
