...
public class Repository {
    ...
    public final String SERVICE_REPOSITORY_DIR = ServletContextParameterMap
	    .getParameterValue(ContextParameter.USER_DIRECTORY_PATH)
	    + "repository/services/";
    public final String SOURCE_REPOSITORY_DIR = ServletContextParameterMap
	    .getParameterValue(ContextParameter.USER_DIRECTORY_PATH)
	    + "repository/sources/";
    public final String TRIPLE_DATASET_DIR = ServletContextParameterMap
	    .getParameterValue(ContextParameter.USER_DIRECTORY_PATH)
	    + "repository/dataset/";

    // @param LANG: The language in which to write the model is specified by the
    // lang argument.
	...
	if (_InternalInstance == null) {
	    ...
	    File serviceRepository = new File(
		    _InternalInstance.SERVICE_REPOSITORY_DIR);
	    if (!serviceRepository.exists())
		serviceRepository.mkdir();

	    File sourceRepository = new File(
		    _InternalInstance.SOURCE_REPOSITORY_DIR);
	...
	// 1) tdbDataset.getNamedModel("urn:x-arq:UnionGraph") does always
	// return the union graph (even when
	// TDB.getContext().set(TDB.symUnionDefaultGraph, true) is NOT set).
	// 2) tdbDataset.getDefaultModel() does always return the 'real' default
	// graph which is different/independent of (1).
	// 3) so only in context of a SPARQL query the'default graph' contains
     ...
     * 
     * @param lang
     *            The language of the file specified by the lang argument.
     *            Predefined values are "RDF/XML", "RDF/XML-ABBREV", "N-TRIPLE",
	...
	if (!file.exists()) {
	    logger.error("cannot find the file/dir at "
	...
	try {
	    ...
	    logger.info("The model " + file.getPath()
		    + " successfully imported to repository");
