...
public class JsonManager {
	...
	String json1 = "{" + "'title': 'Computing and Information systems',"
	...
	String json2 = "{"
		...
		+ "'windDirection': 240," + "'ICAO': 'LESO',"
		+ "'elevation': 8," + "'countryCode': 'ES'," + "'lng': -1.8,"
		+ "'temperature': '22'," + "'dewPoint': '20',"
		+ "'windSpeed': '05'," + "'humidity': 88,"
		+ "'stationName': 'San Sebastian / Fuenterrabia',"
		+ "'datetime': '2011-08-31 18:30:00'," + "'lat': 43.35,"
		+ "'hectoPascAltimeter': 1009" + "}," + "'node2': {"
	...
	String json4 = "{'data':{"
		...
		+ "'value':'http:\\/\\/www.worldweatheronline.com\\/images\\/wsymbols01_png_64\\/wsymbol_0001_sunny.png'"
		+ "}" + "]," + "'winddir16Point':'W',"
		+ "'winddirDegree':'280'," + "'windspeedKmph':'6',"
		+ "'windspeedMiles':'4'" + "}]" + "}" + "}";
    ...
    private static void getLeavesPath(Element element, String prefix,
	    String separator, int depth) {

	if (element.getValueType() == ValueType.SINGLE) {
	    System.out.println("test");
	    element.setFullPath(element.getFullPath() + separator
		    + element.getLocalName(depth));
	} else {
	    int count = 0;
	    for (int i = 0; i < ((ArrayValue) element.getValue()).getElements()
		    .size(); i++) {
		Element e = ((ArrayValue) element.getValue()).getElements()
			.get(i);
    ...
    public static void union(List<List<String>> srcColumns,
	    List<List<List<String>>> srcValues, List<String> columns,
    ...
    public static void union(List<List<String>> srcColumns,
	    List<List<String>> srcTypes, List<List<List<String>>> srcValues,
    ...
    public static void getJsonFlat(String json, List<String> columns,
	    List<List<String>> values) {
