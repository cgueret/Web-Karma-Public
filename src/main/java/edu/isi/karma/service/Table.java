...
public class Table {
    ...
    /**
     * Each service invocation might have different columns than other other
     * invocations. This method integrates all results into one table. For the
     * invocations that don't have a specific column, we put null values in
	...
	for (int i = 0; i < srcAttributes.size(); i++) {
		...
		for (int k = 0; k < resultAttributes.size(); k++) {
		    ...
		    int index = rawAttributeIDs.indexOf(resultAttributes.get(k)
			    .getId());
		    if (index == -1)
			singleValue = null;
		    // singleValue = "";
    ...
    public String asCSV(Character separator, Character quotechar,
	    Character endlinechar) {
	String csv = "";
	if (separator == null)
	    separator = ',';
	if (quotechar == null)
	...
	try {
		...
		for (int i = 0; i < this.headers.size(); i++) {
		    ...
		    csv += quotechar + this.headers.get(i).getName()
			    + quotechar;
		}
		csv += endlinechar;
	    } else {
