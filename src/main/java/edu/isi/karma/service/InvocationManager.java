...
public class InvocationManager {
	...
	if (requestURLs == null || requestURLs.size() == 0)
	    throw new KarmaException(
		    "Cannot model a service without any request example.");
    ...
    public Table getServiceData(boolean includeURL,
	    boolean includeInputAttributes, boolean includeOutputAttributes) {
	...
	List<String> newRowIds = new ArrayList<String>(
		this.serviceData.getRowIds());
	...
	if (headers != null) {
		...
		if (includeInputAttributes
			&& headers.get(i).getIOType() == IOType.INPUT)
		    includingColumns.add(i);
		if (includeOutputAttributes
			&& headers.get(i).getIOType() == IOType.OUTPUT)
		    includingColumns.add(i);
	    }
	}
    ...
    /**
     * This method creates a new service model which includes only the service
     * endpoint, http method, input and output attributes
     * 
     * @return
     */
	...
	// String s1 =
	// "http://colo-vm10.isi.edu:8080/DovetailService/GetSampleData?sourceName=KDD-02-B-TOSIG";
	String s1 = "http://api.geonames.org/neighbourhood?lat=40.78343&lng=-73.96625&username=karma";
	// String s1 =
	// "http://api.geonames.org/postalCodeCountryInfo?username=karma";
	// String s2 =
	// "http://api.geonames.org/neighbourhood?lat=40.7&lng=-73.9&username=karma";
	// String s3 =
	// "http://api.geonames.org/neighbourhood?lat=40.9&lng=-73.9&username=karma";
