...
public class ServicePublisher {
     ...
     */
    public static void publish(Service service, String lang, boolean writeToFile)
	    throws FileNotFoundException {

	_service = service;

	Service existingService = ServiceLoader.getServiceByAddress(_service
		.getAddress());
	...
	if (writeToFile) {
	    ...
	    OutputStreamWriter output = new OutputStreamWriter(
		    new FileOutputStream(service_desc_file));
	...
	// resources
	Resource service_resource = model.createResource(Namespaces.KARMA
		+ "Service");
	Resource variavle_resource = model.createResource(Namespaces.SWRL
		+ "Variable");

	// properties
	Property rdf_type = model.createProperty(Namespaces.RDF, "type");
	Property has_address = model.createProperty(Namespaces.HRESTS,
		"hasAddress");
	Property has_method = model.createProperty(Namespaces.HRESTS,
		"hasMethod");
	Property has_input = model.createProperty(Namespaces.KARMA, "hasInput");
	Property has_output = model.createProperty(Namespaces.KARMA,
		"hasOutput");
	Property has_name = model.createProperty(Namespaces.KARMA, "hasName");
	Property has_variable = model.createProperty(Namespaces.KARMA,
		"hasVariable");
	...
	Literal service_address = model.createTypedLiteral(
		_service.getAddress(), uri_template);
	...
	if (_service.getAddress().length() > 0) {
	    Literal operation_address_literal = model.createTypedLiteral(
		    _service.getAddress(), uri_template);
	...
	if (_service.getVariables() != null)
		...
		Resource my_variable = model.createResource(baseNS
			+ _service.getVariables().get(i).toString());
    ...
    private static Resource addInput(Model model,
	    boolean includeAttributeDetails) {

	Resource my_input = null;

	String baseNS = model.getNsPrefixURI("");
	// resources
	Resource input_resource = model.createResource(Namespaces.KARMA
		+ "Input");

	// properties
	Property rdf_type = model.createProperty(Namespaces.RDF, "type");
	Property has_attribute = model.createProperty(Namespaces.KARMA,
		"hasAttribute");
	Property has_mandatory_attribute = model.createProperty(
		Namespaces.KARMA, "hasMandatoryAttribute");
	Property has_optional_attribute = model.createProperty(
		Namespaces.KARMA, "hasOptionalAttribute");
	...
	if (_service.getInputAttributes() != null) {
		...
		Resource my_attribute = model.createResource(baseNS
			+ att.getId());
    ...
    private static Resource addOutput(Model model,
	    boolean includeAttributeDetails) {
	...
	String baseNS = model.getNsPrefixURI("");
	// resources
	Resource output_resource = model.createResource(Namespaces.KARMA
		+ "Output");
	...
	if (_service.getOutputAttributes() != null) {
		...
		if (att.getRequirement() == AttributeRequirement.NONE)
		    ...
		    my_output
			    .addProperty(has_mandatory_attribute, my_attribute);
		else if (att.getRequirement() == AttributeRequirement.OPTIONAL)
    ...
    private static void addAttributeDetails(Model model, Resource my_attribute,
	    Attribute att) {

	Resource attribute_resource = model.createResource(Namespaces.KARMA
		+ "Attribute");

	Property rdf_type = model.createProperty(Namespaces.RDF, "type");
	Property has_name = model.createProperty(Namespaces.KARMA, "hasName");
	Property is_grounded_in = model.createProperty(Namespaces.HRESTS,
		"isGroundedIn");
	...
	if (att.getIOType() == IOType.INPUT) {
	    ...
	    Literal ground_literal = model.createTypedLiteral(
		    att.getGroundedIn(), rdf_plain_literal);
	    my_attribute.addLiteral(is_grounded_in, ground_literal);
	}

    }

    private static void addModelPart(Model model, Resource resource,
	...
	Resource model_resource = model.createResource(Namespaces.KARMA
		+ "Model");
	Resource class_atom_resource = model.createResource(Namespaces.SWRL
		+ "ClassAtom");
	Resource individual_property_atom_resource = model
		.createResource(Namespaces.SWRL + "IndividualPropertyAtom");
	...
	Property class_predicate = model.createProperty(Namespaces.SWRL,
		"classPredicate");
	Property property_predicate = model.createProperty(Namespaces.SWRL,
		"propertyPredicate");
	Property has_argument1 = model.createProperty(Namespaces.SWRL,
		"argument1");
	Property has_argument2 = model.createProperty(Namespaces.SWRL,
		"argument2");

	Resource my_model = model
		.createResource(baseNS + semanticModel.getId());
	...
	if (semanticModel != null) {
		...
		if (atom instanceof ClassAtom) {
			...
			model.setNsPrefix(classAtom.getClassPredicate()
				.getPrefix(), classAtom.getClassPredicate()
				.getNs());
		    Resource className = model.createResource(classAtom
			    .getClassPredicate().getUriString());
		    r.addProperty(class_predicate, className);

		    Resource arg1 = model.getResource(baseNS
			    + classAtom.getArgument1().getAttOrVarId());
			...
			model.setNsPrefix(propertyAtom.getPropertyPredicate()
				.getPrefix(), propertyAtom
				.getPropertyPredicate().getNs());
		    Resource propertyName = model.createResource(propertyAtom
			    .getPropertyPredicate().getUriString());
		    r.addProperty(property_predicate, propertyName);

		    Resource arg1 = model.getResource(baseNS
			    + propertyAtom.getArgument1().getAttOrVarId());
		    r.addProperty(has_argument1, arg1);

		    Resource arg2 = model.getResource(baseNS
			    + propertyAtom.getArgument2().getAttOrVarId());
	...
	String serviceUri = Service.KARMA_SERVICE_PREFIX
		+ "CDA81BE4-DD77-E0D3-D033-FC771B2F4800" + "#";
	...
	OutputStreamWriter outputService = new OutputStreamWriter(
		new FileOutputStream(service_file));
	OutputStreamWriter outputServiceInput = new OutputStreamWriter(
		new FileOutputStream(service_input_file));
	OutputStreamWriter outputServiceOutput = new OutputStreamWriter(
		new FileOutputStream(service_output_file));

	com.hp.hpl.jena.rdf.model.Model model = ServicePublisher
		.generateModel(service);
	com.hp.hpl.jena.rdf.model.Model inputModel = ServicePublisher
		.generateInputPart(service);
	com.hp.hpl.jena.rdf.model.Model outputModel = ServicePublisher
		.generateOutputPart(service);
