...
public class Invocation {
	...
	if (count == null) {
	    ...
	    return (Attribute.OUTPUT_PREFIX + name + "_" + String.valueOf(count
		    .intValue()));
	}
    }
	...
	if (response.getType().indexOf("xml") != -1) { // XML content
	    // The library has a bug, some values are wrong, e.g., adminCode2:
	    // 061 --> 49
	    ...
	    JsonManager.getJsonFlat(response.getStream(), columns,
		    results.getValues());
	} else {
	...
	for (String c : columns) {
	    Attribute p = new Attribute(getId(c, attributeNameCounter), c,
		    IOType.OUTPUT);
	...
	// FIXME : just to rule out some attributes in the GUI for the paper
	// figure
	// int i = 0;
	...
	try {
	    ...
	    logger.error("Error in invoking the service with request "
		    + this.request.getUrl().toString());

	    this.response.setType("application/json");
	    this.response.setCode(code);
	    this.response.setStream("{\"code\":" + code + ",\"msg\":\""
		    + e.getMessage() + "\"}");
	...
	jointInputAndOutput.getHeaders().add(
		0,
		new Attribute(REQUEST_COLUMN_NAME, REQUEST_COLUMN_NAME,
			IOType.NONE));
	for (int k = 0; k < jointInputAndOutput.getValues().size(); k++)
	    jointInputAndOutput.getValues().get(k)
		    .add(0, this.request.getUrl().toString());
