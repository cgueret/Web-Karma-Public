...
public class URLManager {
    ...
    public static List<URL> getURLsFromJSON(String requestURLsJSONArray)
	...
	for (int i = 0; i < requestURLStrings.size(); i++) {
		...
		throw new KarmaException(
			"To model a service, all request examples should have the same endpoint.");
	    urls.add(url);
	}
    ...
    /**
     * Example: "http://www.test.com/getVideos?user=demo" returns
     * "http://www.test.com/getVideos" Example: "http://www.test.com/getVideos/"
     * returns "http://www.test.com/getVideos"
     * 
     * @attribute url
     * @return
     */
    ...
    /**
     * Example: "http://www.test.com/getVideos?user=demo" returns
     * "http://www.test.com" Example: "http://www.test.com/getVideos/" returns
     * "http://www.test.com/getVideos/"
     * 
     * @attribute url
     * @return
     */
	...
	if (operationName.indexOf("/") != -1)
	    operationName = operationName.substring(
		    operationName.lastIndexOf("/") + 1, operationName.length());
    ...
    private static boolean verifyAttributeExtraction(URL url,
    ...
    public static List<Attribute> getQueryAttributes(String urlString)
	...
	try {
		...
		for (String attribute : query.split("&")) {
			...
			// remember that we can have multiple values for a
			// single attributeeter
			// example:
			// /path/to/my/resource?attribute1=value1&attribute1=value2&attribute1=value3
			// we are currently ignoring these types of urls

			if (values.size() > 0) {
			    attributeName = key;
			    attributeId = getId(attributeName,
				    attributeNameCounter);
			    attributeValue = values.get(0);
			    Attribute p = new Attribute(attributeId,
				    attributeName, IOType.INPUT, attributeValue);
			    attributeList.add(p);
	    ...
	    logger.debug("Attributes extracted successfully from "
		    + url.toString());
	...
	if (count == null) {
	    ...
	    return (Attribute.INPUT_PREFIX + name + "_" + String.valueOf(count
		    .intValue()));
	}
    }
     ...
     * @throws MalformedURLException
     */
    public static List<String> extractAttributeNames(List<String> urlList)
	    throws MalformedURLException {
	...
	for (int i = 0; i < urlList.size(); i++) {
		...
		if (key.trim().length() > 0
			&& attributesNameList.indexOf(key) == -1) {
    ...
    public static List<List<String>> attributeValuesInTable(
	    List<String> urlList, List<String> attributeNames)
	    throws MalformedURLException {
	...
	for (int i = 0; i < urlList.size(); i++) {
		...
		if (p.getValue() != null && p.getValue().trim().length() > 0)
		    if (index != -1)
			if (allAttributes.get(index).indexOf(
				p.getValue().trim()) == -1)
    ...
    public static void printAttributesTabular(List<String> attributeNames,
	    List<List<String>> attributeValuesInTable) {
	...
	try {
		...
		for (int j = 0; j < attributeValuesInTable.get(i).size(); j++) {
		    ...
		    System.out.write(attributeValuesInTable.get(i).get(j)
			    .getBytes());
		    System.out.write("\n".getBytes());
		}
		System.out.write("\n".getBytes());
    ...
    public static String printAttributesCSV(List<String> urlList,
    ...
    public static void printAttributesSets(List<String> attributeNames,
	    List<List<String>> attributeValuesInSets) {
    ...
    public static String createApiLink(String apiEndPoint,
	...
	// apiEndPoint =
	// "http://sandbox.api.shopping.com/publisher/3.0/rest/generalsearch?";
	...
	// List<String> urlList = getUrlListFromFile(pathPrefix + serviceName +
	// ".txt");
	//
	// List<String> attributeNames = extractAttributeeterNames(urlList);
	// List<List<String>> attributeValuesInTable =
	// attributeValuesInTable(urlList, attributeNames);
	// List<List<String>> attributeValuesInSets =
	// attributeValuesInSets(urlList, attributeNames);
	// List<List<String>> crossProductTable =
	// Permutation.permuteAll(attributeValuesInSets);
	//
	// // printAttributesCSV(urlList, attributeNames,
	// attributeValuesInTable, pathPrefix + serviceName + ".csv");
	...
	//
	// printAttributesCSV(urlList, attributeNames, crossProductTable,
	// pathPrefix + serviceName + "-cross.csv");
	//
	// String url = "";
	// String output = "";
	// // for (int i = 0; i < 10; i++) {
	...
	// url = createApiLink(apiEndPoint, attributeNames,
	// crossProductTable.get(i));
	// System.out.println(url);
	// //url =
	// "http://maps.googleapis.com/maps/api/geocode/json?address=1600 Amphitheatre Parkway, Mountain View, CA&sensor=true";
	// // url =
	// "http://weather.yahooapis.com/forecastrss?w=location&u=c&p=000";
