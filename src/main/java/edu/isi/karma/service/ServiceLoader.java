...
public class ServiceLoader {
	...
	String service_id = uri.substring(uri.lastIndexOf("/") + 1,
		uri.length() - 1);
	String dir = Repository.Instance().SERVICE_REPOSITORY_DIR;
	String fileName = service_id
		+ Repository.Instance().getFileExtension(
			Repository.Instance().LANG);
	File f = new File(dir + fileName);

	try {
	    if (f.exists()) {
		if (!f.delete())
		    logger.debug("The file " + fileName
			    + " cannot be deleted from " + dir);
		else
		    logger.debug("The file " + fileName
			    + " has been deleted from " + dir);
	    } else
		logger.debug("The file " + fileName + " does not exist in "
			+ dir);
	} catch (Throwable t) {
	    logger.debug("cannot delete the file " + fileName + " from " + dir
     ...
     * 
     * @param limit
     *            : maximum number of results, null value means all the services
     * @return
     */
	...
	// Create a new query
	String queryString = "PREFIX " + Prefixes.KARMA + ": <"
		+ Namespaces.KARMA + "> \n" + "PREFIX " + Prefixes.HRESTS
		+ ": <" + Namespaces.HRESTS + "> \n"
		+ "SELECT ?s ?name ?address \n" + "WHERE { \n" + "      ?s a "
		+ Prefixes.KARMA + ":Service . \n" + "      OPTIONAL {?s "
		+ Prefixes.HRESTS + ":hasAddress ?address .} \n"
		+ "      OPTIONAL {?s " + Prefixes.KARMA
		+ ":hasName ?name .} \n" + "      } \n";
	...
	if (serviceLimit != null) {
	    ...
	    queryString += "LIMIT "
		    + String.valueOf(serviceLimit.intValue() + "\n");
	}
	...
	try {
		...
		RDFNode name = soln.get("name"); // Get a result variable by
						 // name.
		RDFNode address = soln.get("address"); // Get a result variable
						       // by name.
		...
		service_id = service_uri.substring(
			service_uri.lastIndexOf("/") + 1,
			service_uri.length() - 1);
		...
		if (service_id.trim().length() > 0)
		    serviceList.add(new Service(service_id, service_name,
			    service_address));
		else
    ...
    /**
     * returns all the services in the repository along with their complete
     * information probably this method is not useful since fetching all the
     * services with their complete information takes long time.
     * 
     * @return
     */
    ...
    /**
     * Searches the repository to find the services that the semantic model
     * parameter is contained in their input model.
     * 
     * @param semanticModel
     *            The input model whose pattern will be searched in the
     *            repository
     * @param ioType
     *            declares which one of the service input or service output will
     *            be tested for matching.
     * @param operationsLimit
     *            maximum number of operations that will be fetched
     * @return a hashmap of all found services and a mapping from the found
     *         service parameters to the model parameters. This help us later to
     *         how to join the model's corresponding source and the matched
     *         service
     */
    public static Map<Service, Map<String, String>> getServicesByInputPattern(
	    edu.isi.karma.service.Model semanticModel, Integer serviceLimit) {
	...
	Map<Service, Map<String, String>> servicesAndMappings = new HashMap<Service, Map<String, String>>();

	Map<String, Map<String, String>> serviceIdsAndMappings = semanticModel
		.findInServiceInputs(Repository.Instance().getModel(),
			serviceLimit);
	...
	for (String serviceId : serviceIdsAndMappings.keySet()) {
	    ...
	    if (m != null)
		servicesAndMappings.put(getServiceFromJenaModel(m),
			serviceIdsAndMappings.get(serviceId));
	}
    ...
    /**
     * Searches the repository to find the services that the semantic model
     * parameter is contained in their output model.
     * 
     * @param semanticModel
     *            The input model whose pattern will be searched in the
     *            repository
     ...
     *         service
     */
    public static Map<Service, Map<String, String>> getServicesByOutputPattern(
	    edu.isi.karma.service.Model semanticModel, Integer serviceLimit) {
	...
	Map<String, Map<String, String>> serviceIdsAndMappings = semanticModel
		.findInServiceOutputs(Repository.Instance().getModel(),
			serviceLimit);
     ...
     * contained in the semantic model parameter. Note that the services in the
     * return list only include the operations that match the model parameter.
     * 
     * @param semanticModel
     *            The input model whose pattern will be searched in the
     *            repository
	...
	for (Service service : serviceList) {
	    ...
	    Map<String, Map<String, String>> serviceIdsAndMappings = m
		    .findInJenaModel(jenaModel, null);
		...
		String key = itr.next();
		servicesAndMappings
			.put(service, serviceIdsAndMappings.get(key));
	    }
	}
    ...
    /**
     * Searches the repository to find the services whose output model is
     * contained in the semantic model parameter. Note that the services in the
     * return list only include the operations that match the model parameter.
     * 
     * @param semanticModel
	...
	String queryString = "PREFIX " + Prefixes.KARMA + ": <"
		...
		+ "WHERE { \n" + "      ?s a " + Prefixes.KARMA
		+ ":Service . \n" + "      ?s " + Prefixes.HRESTS
		+ ":hasAddress \"" + address + "\"^^hrests:URITemplate . \n"
		+ "      } \n";
	...
	try {
		...
		logger.info("Service with id " + x.toString()
			+ " has been found with the address " + address);
		break;
	    }

	    return serviceURI;
	} catch (Exception e) {
	    logger.info("Exception in finding a service with the address "
		    + address + " in service repository.");
	    return null;
	} finally {
	    qexec.close();
	}
	...
	// service local id
	service_id = service_uri.substring(service_uri.lastIndexOf("/") + 1,
		service_uri.length() - 1);
	logger.debug("service id: " + service_id);

	Property has_address_property = model.getProperty(Namespaces.HRESTS
		+ "hasAddress");
	Property has_name_property = model.getProperty(Namespaces.KARMA
		+ "hasName");
	Property has_method_property = model.getProperty(Namespaces.HRESTS
		+ "hasMethod");
	Property has_input_property = model.getProperty(Namespaces.KARMA
		+ "hasInput");
	Property has_output_property = model.getProperty(Namespaces.KARMA
		+ "hasOutput");
	...
	// service name
	nodeIterator = model.listObjectsOfProperty(service_resource,
		has_name_property);
	...
	if (nodeIterator.hasNext() && (node = nodeIterator.next()).isResource()) {
	    inputAttributes = getAttributes(model, node.asResource(),
		    IOType.INPUT);
	...
	nodeIterator = model.listObjectsOfProperty(service_resource,
		has_output_property);
	if (nodeIterator.hasNext() && (node = nodeIterator.next()).isResource()) {
	    outputAttributes = getAttributes(model, node.asResource(),
		    IOType.OUTPUT);
    ...
    private static List<String> getVariables(Model model,
	    Resource service_resource) {

	Property has_variable_property = model.getProperty(Namespaces.KARMA
		+ "hasVariable");
    ...
    private static List<Attribute> getAttributes(Model model,
	    Resource io_resource, String ioType) {

	Property has_attribute_property = model.getProperty(Namespaces.KARMA
		+ "hasAttribute");
	Property has_mandatory_attribute_property = model
		.getProperty(Namespaces.KARMA + "hasMandatoryAttribute");
	Property has_optional_attribute_property = model
		.getProperty(Namespaces.KARMA + "hasOptionalAttribute");
	...
	// hasAttribute
	nodeIterator = model.listObjectsOfProperty(io_resource,
		has_attribute_property);
	while (nodeIterator.hasNext()) {
	    node = nodeIterator.next();
	    ...
	    attList.add(getAttribute(model, node.asResource(), ioType,
		    AttributeRequirement.NONE));
	}

	// hasMandatoryAttribute
	nodeIterator = model.listObjectsOfProperty(io_resource,
		has_mandatory_attribute_property);
	while (nodeIterator.hasNext()) {
	    node = nodeIterator.next();
	...
	nodeIterator = model.listObjectsOfProperty(io_resource,
		has_optional_attribute_property);
	while (nodeIterator.hasNext()) {
	    node = nodeIterator.next();
    ...
    private static Attribute getAttribute(Model model, Resource att_resource,
	    String ioType, AttributeRequirement requirement) {
	...
	Property has_name_property = model.getProperty(Namespaces.KARMA
		+ "hasName");
	Property is_gounded_in_property = model.getProperty(Namespaces.HRESTS
		+ "isGroundedIn");
	...
	// attribute name
	nodeIterator = model.listObjectsOfProperty(att_resource,
		has_name_property);
	if (nodeIterator.hasNext() && (node = nodeIterator.next()).isLiteral()) {
	...
	if (att_groundedIn.length() > 0)
	    att = new Attribute(att_id, att_resource.getNameSpace(), att_name,
		    ioType, requirement, att_groundedIn);
	else
	    att = new Attribute(att_id, att_resource.getNameSpace(), att_name,
		    ioType, requirement);
    ...
    private static edu.isi.karma.service.Model getSemanticModel(Model model,
	    Resource io_resource) {

	Property has_model_property = model.getProperty(Namespaces.KARMA
		+ "hasModel");
	Property has_atom_property = model.getProperty(Namespaces.KARMA
		+ "hasAtom");
	...
	if (!nodeIterator.hasNext()
		|| !(modelNode = nodeIterator.next()).isResource()) {
	    logger.info("There is no model resource.");
	    return null;
	}

	edu.isi.karma.service.Model semanticModel = new edu.isi.karma.service.Model(
		modelNode.asResource().getLocalName());
	List<Atom> atoms = new ArrayList<Atom>();

	// hasAtom
	nodeIterator = model.listObjectsOfProperty(modelNode.asResource(),
		has_atom_property);
	while (nodeIterator.hasNext()) {
	...
	if (!nodeIterator.hasNext()
		|| !(node = nodeIterator.next()).isResource()) {
	...
	Resource attribute = ResourceFactory.createResource(Namespaces.KARMA
		+ "Attribute");
	Resource variable = ResourceFactory.createResource(Namespaces.SWRL
		+ "Variable");

	Property class_predicate_property = model.getProperty(Namespaces.SWRL
		+ "classPredicate");
	Property argument1_property = model.getProperty(Namespaces.SWRL
		+ "argument1");
    ...
    private static PropertyAtom getPropertyAtom(Model model,
	    Resource atom_resource) {
	...
	Property property_predicate_property = model
		.getProperty(Namespaces.SWRL + "propertyPredicate");
	Property argument1_property = model.getProperty(Namespaces.SWRL
		+ "argument1");
	Property argument2_property = model.getProperty(Namespaces.SWRL
		+ "argument2");
    ...
    private static boolean isInstanceOfTheClass(Resource resource,
	    Resource class_resource) {
	Property type_property = ResourceFactory.createProperty(Namespaces.RDF
		+ "type");
	...
	URI featurePredicatName = new URI(geoOntology + "Feature", geoOntology,
		"geo");
	URI latPredicatName = new URI(geoOntology + "lat", geoOntology, "geo");
	URI lngPredicatName = new URI(geoOntology + "long", geoOntology, "geo");

	ClassAtom c1 = new ClassAtom(featurePredicatName, new Argument("arg1",
		"arg1", ArgumentType.ATTRIBUTE));
	PropertyAtom p1 = new PropertyAtom(latPredicatName, new Argument(
		"arg1", "arg1", ArgumentType.ATTRIBUTE), new Argument("arg2",
		"arg2", ArgumentType.ATTRIBUTE));
	PropertyAtom p2 = new PropertyAtom(lngPredicatName, new Argument(
		"arg1", "arg1", ArgumentType.ATTRIBUTE), new Argument("arg3",
		"arg3", ArgumentType.ATTRIBUTE));
	// ClassAtom c2 = new ClassAtom(featurePredicatName, new
	// Argument("arg2", "arg2", ArgumentType.ATTRIBUTE));
	...
	Map<Service, Map<String, String>> servicesAndMappings = getServicesWithInputContainedInModel(
		semanticModel, null);
	...
	for (Service s : servicesAndMappings.keySet()) {
		...
		System.out.println(str + "-------"
			+ servicesAndMappings.get(s).get(str));
	}
