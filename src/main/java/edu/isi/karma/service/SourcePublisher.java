...
public class SourcePublisher {
    ...
    // I tried to do it in a nicer way but couldn't figure out how to add it to
    // the Attribute
    public SourcePublisher(Source source, String sourceDescription,
	    RepFactory factory, List<String> transformationCommandJSON,
     ...
     */
    public void publish(String lang, boolean writeToFile)
	    throws FileNotFoundException {
	...
	// resources
	Resource source_resource = model.createResource(Namespaces.KARMA
		+ "Source");
	...
	Property has_columnName = model.createProperty(Namespaces.KARMA,
		"hasColumnName");
	...
	if (this.source.getVariables() != null)
		...
		Resource my_variable = model.createResource(baseNS
			+ this.source.getVariables().get(i).toString());
	...
	if (this.source.getAttributes() != null) {
		...
		my_attribute.addProperty(has_columnName,
			factory.getHNode(att.gethNodeId()).getColumnName());
	...
	// Add transformations
	Property has_columnTransformation = model.createProperty(
		Namespaces.KARMA, "hasColumnTransformation");
	...
	if (sourceInfo != null) {
	    Map<InfoAttribute, String> attributeValueMap = sourceInfo
		    .getAttributeValueMap();
	    for (InfoAttribute attr : attributeValueMap.keySet()) {
		Property attrProp = model.createProperty(Namespaces.KARMA,
			"hasSourceInfo_" + attr.name());
    ...
    public void addModelPart(Model model, Resource resource,
    ...
    /*
     * public static void main(String[] args) { Source source = new
     * Source("mySource", Test.getGeoNamesNeighbourhoodTree()); source.print();
     * SourcePublisher sourcePublisher = new SourcePublisher(source,""); try {
     * sourcePublisher.publish(Repository.Instance().LANG, true); } catch
