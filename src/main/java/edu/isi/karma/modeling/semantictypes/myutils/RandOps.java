...
/**
 * This class provides utility methods for performing randomness based
 * operations, such as, generating random numbers within a range.
 * 
 * @author amangoel
 * 
 */
public class RandOps {
    ...
    /**
     * @param limit
     *            Max limit. Numbers will be returned from 0 (inclusive) and
     *            limit (exclusive)
     * @param nNumbers
     *            Number of random numbers to be returned (cannot be greater
     *            than limit)
     * @return A list of integers randomly selected within the above mentioned
     *         limit
     */
    public static ArrayList<Integer> uniqSortedRandNums(int limit, int nNumbers) {
	// returns nNumbers unique random numbers between 0 (inclusive) and
	// limit (exclusive)
	if (limit < nNumbers) {
     ...
     * @param originalList
     *            The original list containing all the items
     * @param selectedList
     *            The list in which randomly selected items will be returned
     * @param numItems
     *            Number of items to be randomly selected from originallist
     */
    public static <T> void getRandomlySelectedItemsFromList(
	    List<T> originalList, List<T> selectedList, int numItems) {
