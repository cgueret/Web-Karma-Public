...
/**
 * This class performs the task of finding the step size for the gradient during
...
public class BacktrackingLineSearch {
    ...
    public BacktrackingLineSearch(CRFModelAbstract crfModel,
	    GlobalDataAbstract globalData) {
	...
	if (searchDirNorm > Constants.BACKTRACKINGLINESEARCH_MAX_STEP) {
	    double ratio = Constants.BACKTRACKINGLINESEARCH_MAX_STEP
		    / searchDirNorm;
	    for (int i = 0; i < dim; i++)
		searchDir[i] *= ratio;
	}
	...
	double tmp = 0.0;
	for (int i = 0; i < dim; i++) {
	    tmp = Math.abs(searchDir[i]
		    / Math.max(Math.abs(currWeights[i]), 1.0));
	    if (tmp > test)
		test = tmp;
	}
	...
	lam1 = 1.0;
	while (true) {
	    Prnt.prn("BacktrackingLineSearch iteration #" + iteration
		    + " and lambda = " + lam1);
		...
		double a = (rhs1 / (lam1 * lam1) - rhs2 / (lam2 * lam2))
			/ (lam1 - lam2);
		double b = ((-rhs1 * lam2 / (lam1 * lam1)) + rhs2 * lam1
			/ (lam2 * lam2))
			/ (lam1 - lam2);
		if (a == 0.0)
		    tmplam = -slope / (2.0 * b);
		...
		Prnt.prn("Returning because tmplam = " + tmplam
			+ " < lammin = " + lammin);
		return 0.0;
	    }
