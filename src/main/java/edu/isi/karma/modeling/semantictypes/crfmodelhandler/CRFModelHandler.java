...
/**
 * This class is an API to the mycrf package. It allows instantiating CRF
 * models, training them and using them for prediction.
 * 
 * @author amangoel
 * 
 */
public class CRFModelHandler {
    ...
    // ***********************************************************************************************
    /**
     * @author amangoel The ColumnFeature enum with members representing the
     *         possible features that could be passed.
     * 
     */
    public enum ColumnFeature {
	ColumnHeaderName, TableName
    };
    ...
    // ***********************************************************************************************
    /**
     * @author amangoel This internal class represents an example.
     */
	 ...
	 *            Associated ColumnFeatures It takes in a collection of
	 *            feature values for each ColumnFeature, but only picks the
	 *            first value to store in the Example, as I don't see yet
	 *            why more than one String should be associated with a
	 *            ColumnFeature.
	 */
	public Example(String exampleString,
		...
		for (Map.Entry<ColumnFeature, Collection<String>> entry : columnFeatures
			.entrySet()) {
			...
			if (featureValue != null) {
			    this.columnFeatures.put(entry.getKey(),
				    featureValue);
			}
		    }
		}
	 ...
	 *            A ColumnFeature
	 * @param featureValue
	 *            Corresponding Value to the ColumnFeature
	 */
	public void addColumnFeature(ColumnFeature columnFeature,
		String featureValue) {
	...
	/**
	 * @param colFeature
	 *            ColumnFeature for which the value is required.
	 * @return The value corresponding to the ColumnFeature, or null if the
	 *         example doesn't have the passed ColumnFeature. Checking the
	 *         return value from this method is therefore important.
	 */
    ...
    static Logger logger = LoggerFactory.getLogger(CRFModelHandler.class
	    .getSimpleName());
    static final int MAX_FFs_PER_LABEL = 50;
    static final int MAX_EXAMPLES_PER_LABEL = 50;

    /**
     * Making the empty constructor private to prevent instantiation of this
     * class. This class should only be used to access its static methods.
     */
    ...
    /**
     * Adds the passed list of examples to the model. Regenerates 100 feature
     * functions to represent the label, if examples of this label already exist
     * in the model.
     * 
     * @param label
     ...
     * @param columnFeatures
     *            Map of column features.
     * @return True if success, else False
     */
    public synchronized boolean addOrUpdateLabel(String label,
	...
	if (examples.size() == 0) {
	    Prnt.prn("@examples list contains forbidden characters only. The allowed characters are "
		    + allowedCharacters);
	    return false;
	}
	// if label does not already exist in the model, add new label. Also,
	// add an entry in the map for the new label.
	...
	// make sure that we consider MAX_EXAMPLES_PER_LABEL examples for
	// training.
	...
	if (labelToExamplesMap.get(label).size() <= MAX_EXAMPLES_PER_LABEL) {
	    ...
	    RandOps.getRandomlySelectedItemsFromList(
		    labelToExamplesMap.get(label), selectedExamples,
	...
	for (Example selectedExample : selectedExamples) {
	    ...
	    newGraph = new GraphFieldOnly(selectedExample.exampleString, label,
		    new ArrayList<String>(tmpFeatures), globalData);
	    globalData.trainingGraphs.add(newGraph);
	    allFeatures.addAll(tmpFeatures);
	}
	// if the total number of features is > NUM_FFs, then randomly select
	// NUM_FFs from them.
	selectedFeatures = new HashSet<String>(allFeatures);
	if (selectedFeatures.size() > MAX_FFs_PER_LABEL) {
	    selectFeatureSetWithWeightedProbability(allFeatures,
		    selectedFeatures);
	}
	...
	optimizationObject = new OptimizeFieldOnly(globalData.crfModel,
		globalData);
    ...
    /**
     * @param label
     *            The label for which examples are being requested.
     * @param examples
     *            The list argument that will be used to return the list of
     *            examples in the model for the supplied label.
	...
	if (!globalData.labels.contains(label)) {
	    Prnt.prn("CRFModelHandler.getExamplesForLabel: Label " + label
     ...
     * @return True, if successful, else False
     */
    public boolean predictLabelForExamples(List<String> examples,
	    int numPredictions, List<String> predictedLabels,
	    List<Double> confidenceScores, List<double[]> exampleProbabilities,
	    Map<ColumnFeature, Collection<String>> columnFeatures) {
	...
	if (examples == null || examples.size() == 0 || numPredictions <= 0
		|| predictedLabels == null || confidenceScores == null) {
	...
	// the label that gets highest accumulated probability, is the most
	// likely label for all examples combined
	for (String example : examples) {
	    ...
	    exampleGraph = new GraphFieldOnly(sanitizedExample, null,
		    new ArrayList<String>(features), globalData);
	    probabilitiesForExample = MAPPredictor
		    .probabilitiesForLabels(exampleGraph);
	    Matrix.plusEquals(columnProbabilities, probabilitiesForExample, 1.0);
	    if (exampleProbabilities != null) {
		exampleProbabilitiesFullList
			.add(newListFromDoubleArray(probabilitiesForExample));
	    }
	}
	// the sum of all values in the probabilies array is going to be
	// examples.size()
	...
	// Sort both lists such that labels are listed according to their
	...
	for (int index = 0; index < globalData.labels.size()
		&& index < numPredictions; index++) {
		...
		for (int i = 0; i < examples.size(); i++) {
		    exampleProbabilities.get(i)[index] = exampleProbabilitiesFullList
			    .get(i).get(li);
		}
	    }
	}
    ...
    /**
     * @param modelFile
     *            The path of the file from which the model should be read.
     * @return True is successfully read. False, otherwise. This function takes
     *         the path of file as input and creates an environment that
     *         consists of globalData, crfModel, list of examples of each label,
     *         etc. It reads an empty file also.
     */
	...
	if (emptyFile) {
		...
		for (int labelNumber = 0; labelNumber < numLabels; labelNumber++) {
			...
			Prnt.prn("The label "
				+ newLabel
				+ " was already added to the model. "
		    ...
		    br.readLine(); // consuming the empty line after each list
				   // of label and its examples
		}
		// Creating trainingGraphs for MAX_EXAMPLES_PER_LABEL num of
		// examples
		globalData.trainingGraphs = new ArrayList<GraphInterface>();
		selectedExamples = new ArrayList<CRFModelHandler.Example>();
		for (String lbl : globalData.labels) {
			...
			RandOps.getRandomlySelectedItemsFromList(allExamples,
				selectedExamples, MAX_EXAMPLES_PER_LABEL);
		    }
		    for (Example example : selectedExamples) {
			featureSet(example, features);
			globalData.trainingGraphs.add(new GraphFieldOnly(
				example.exampleString, lbl,
				new ArrayList<String>(features), globalData));
		    }
		}
		// starting to read in feature-functions and their weights. the
		// first line is the number of such ffs.
		...
		for (int ffNumber = 0; ffNumber < numFFs; ffNumber++) {
		    ...
		    if (line.length() == 0) {
			Prnt.prn("While reading "
				+ numFFs
				+ " feature functions, we encountered an empty line. This is an error. "
		    ...
		    ffs.add(new LblFtrPair(globalData.labels
			    .indexOf(lineParts[0]), lineParts[1]));
     ...
     *         method removes all labels from the CRF model. This is effectively
     *         same as setting the model to a state, where an empty file has
     *         been read for the first time. Since, each change in the model is
     *         immediately reflected in the model file, this method also
     *         completely clears the model file.
     * 
     */
	...
	// Therefore, all ffs that had labelIndex > the index of the label to be
	// removed
	// should have their
     ...
     * @param cleanedList
     *            List with examples that dont have unallowed chars and others
    ...
    /**
     * @param columnName
     *            The value passed for the ColumnFeature ColumnHeaderName
     * @param features
     *            The set in which the features extracted about this value will
     *            be returned.
     */
    private void extractFeaturesFromColumnName(String columnName,
	    HashSet<String> features) {
    ...
    /**
     * @param tableName
     *            The value passed for the ColumnFeature TableName
     * @param features
     *            The set in which the features extracted about this value will
     *            be returned.
     */
    private void extractFeaturesFromTableName(String tableName,
	    HashSet<String> features) {
	ArrayList<String> parts;
	HashSet<String> nonDupParts;
	parts = new ArrayList<String>();
    ...
    /**
     * @param field
     *            A string from which syntactic features will be extracted
     * @param features
     *            The arg used to return those features.
     */
    ...
    /**
     * @param example
     *            The example for which the features have to extracted
     * @param features
     *            The arg used to return those features.
     */
	...
	featureValue = example
		.getValueForColumnFeature(ColumnFeature.ColumnHeaderName);
	if (featureValue != null) {
	...
	featureValue = example
		.getValueForColumnFeature(ColumnFeature.TableName);
	if (featureValue != null) {
    ...
    /**
     * @param field
     *            Field for which features are to be extracted
     * @param columnFeatures
     *            The columnFeatures of the field.
     * @param features
     *            A set used to return the features. This method just uses the
     *            first string in every collection to construct an Example. It
     *            then uses featureSet(Example, HashSet<String>) method to
     *            return the features for this created example.
     */
    private void featureSet(String field,
	    Map<ColumnFeature, Collection<String>> columnFeatures,
	    HashSet<String> features) {
	Example example;
     ...
     * @return A list containing the same doubles in the same order A utility
     *         method to get a new list having the same values as an array
     */
     ...
     * @throws Exception
     *             Mainly IOException This method starts from wherever the
     *             BufferedReader is and keeps reading till it has parsed an
     *             entire Example. Then it returns it.
     */
	...
	while (true) {
		...
		if (contentLen == -1) {
		    ...
		    for (int i = 0; i < contentLen; i++) {
			c = (char) br.read();
			columnFeatureStringAndValue = columnFeatureStringAndValue
				+ c;
		    }
		    columnFeatureString = columnFeatureStringAndValue
			    .split(":")[0];
		    columnFeatureValue = columnFeatureStringAndValue
			    .substring(columnFeatureString.length() + 1); // to
									  // ignore
									  // the
									  // colon
		    columnFeature = null;
		    try {
			columnFeature = Enum.valueOf(ColumnFeature.class,
				columnFeatureString);
		    } catch (Exception e) {
			Prnt.prn("Parsing of file failed. There is no ColumnFeature called "
				+ columnFeatureString + ".");
			return null;
		    }
		...
		Prnt.prn("Parsing of file failed because found a character other than space or newline after a column feature. The charcter is "
			+ ((int) c));
		return null;
	    }
	}
    ...
    /**
     * @param br
     *            BufferedReader reading the model file.
    ...
    /**
     * @param labelIndex
     *            The labelIndex for which the feature functions will be
     *            reselected.
     * @param newFeatureSet
     *            The new set of features to be included. This method takes in a
     *            set of features and a labelIndex. It removes all existing
     *            feature functions for this labelIndex. It adds new feature
     *            functions for the features supplied. It then sets the weights
     *            for feature functions that already existed to their old
     *            values. It sets the weights for all new feature functions to
     *            zero.
     */
	...
	// from the existing ffs of this label, if any of them have a selected
	// feature, then add it to the other ffs and its learned weight
	...
	// create new ffs for all other selected features and add zero as their
	// weight
    ...
    private void selectFeatureSetWithWeightedProbability(
	    List<String> allFeatures, Set<String> selectedFeatureSet) {
	...
	try {
		...
		for (Example example : examples) {
		    bw.write(example.exampleString.length() + " "
			    + example.exampleString);
		    for (Map.Entry<ColumnFeature, String> entry : example.columnFeatures
			    .entrySet()) {
			if (entry.getValue() != null) {
			    String featureValue;
			    featureValue = entry.getKey().toString() + ":"
				    + entry.getValue();
			    bw.write(" " + featureValue.length() + " "
				    + featureValue);
			}
		    }
		    bw.write("\n");
		...
		LblFtrPair ff;
		ff = globalData.crfModel.ffs.get(ffIndex);
		bw.write(globalData.labels.get(ff.labelIndex) + " "
			+ ff.feature + " "
			+ globalData.crfModel.weights[ffIndex] + "\n");
	    }
	    bw.close();
	    return true;
	} catch (Exception e) {
	    Prnt.prn("Writing the model to file "
		    + file
		    + " failed. The file can be inconsistent with the model in memory until it is successfully written.");
	    return false;
	}
    }
     ...
     * @param parts
     *            The list in which the parts will be returned
...
/*
 * 
 * public static boolean getWeightedFeatureFunctionSums(String example,
 * Map<ColumnFeature, Collection<String>> columnFeatures, List<Double> sums) {
 * GraphFieldOnly exampleGraph ; HashSet<String> features; double[] ffSums;
 * MAPFieldOnly mapPredictor; features = new HashSet<String>();
 * featureSet(example, columnFeatures, features); exampleGraph = new
 * GraphFieldOnly(example, null, new ArrayList<String>(features), globalData) ;
 * mapPredictor = new MAPFieldOnly(globalData); ffSums =
 * mapPredictor.weightedFeatureFunctionSums(exampleGraph); sums.clear();
 * for(double sum : ffSums) { sums.add(sum); } return true; }
 * 
 * 
 * private static boolean addOrUpdateLabel(String label, List<String> examples)
 * { if (file == null) { Prnt.prn(
 * "CRF Model is not ready, either because it was never read or an error happened while reading it previously. Please try reading the model file again."
 * ); return false ; } else { return addOrUpdateLabel(label, examples, null) ; }
 * }
 * 
 * 
 * 
 ...
 * 
 * @param predictedLabels - the argument in which the ordered list of labels is
 * returned. the size of this list could be smaller than numPredictions if there
 * aren't that many labels in the model already
 * 
 * @param confidenceScores - the probability of the examples belonging to the
 * labels returned.
 * 
 * @return private static boolean predictLabelForExamples( List<String>
 * examples, int numPredictions, List<String> predictedLabels, List<Double>
 * confidenceScores ) { if (CRFModelHandler.file == null) { Prnt.prn(
 * "CRF Model is not ready, either because it was never read or an error happened while reading it previously. Please try reading the model file again."
 * ); return false ; } else { return predictLabelForExamples(examples,
 * numPredictions, predictedLabels, confidenceScores, null) ; } }
 * 
 * @param examples - list of examples of an unknown type
 * 
 * @param numPredictions - required number of predictions in descending order
 ...
 * each example, in the same order, a double array that contains the probability
 * of belonging to the labels returned in predictedLabels.
 * 
 * @return private static boolean predictLabelForExamples( List<String>
 * examples, int numPredictions, List<String> predictedLabels, List<Double>
 * confidenceScores, List<double[]> exampleProbabilities ) { if
 * (CRFModelHandler.file == null) { Prnt.prn(
 * "CRF Model is not ready, either because it was never read or an error happened while reading it previously. Please try reading the model file again."
 * ); return false ; } else { return predictLabelForExamples(examples,
 * numPredictions, predictedLabels, confidenceScores, exampleProbabilities,
 * null) ; } }
 */
