...
/**
 * This class represents the large numbers (as large as 10^1000) which are
 * generated due to exponential computations. It supports basic operations such
...
public class LargeNumber {
	...
	if (Math.abs(n1.exp - n2.exp) > 300 && n1.mantissa != 0.0
		&& n2.mantissa != 0.0) {
	    // Prn.prn("The difference between the exponentials is more than 300. n1 = "
	    // + n1 + " and n2 = " + n2) ;
	    if (n1.exp > n2.exp)
		return n1;
	    else
     ...
     * LargeNumber total = new LargeNumber(1.0, 0) ; while(exponent > 300) {
     * exponent-=300 ; double d = Math.pow(Math.E, 300) ; LargeNumber ln = new
     * LargeNumber(d) ; total = LargeNumber.multiply(total, ln) ; } LargeNumber
     * ln = new LargeNumber(Math.pow(Math.E, exponent)) ; total =
     * LargeNumber.multiply(total, ln) ; total.normalize(); return total ; }
     */
	...
	if (Math.abs(this.exp - largeNumber2.exp) > 300 && this.mantissa != 0.0
		&& largeNumber2.mantissa != 0.0) {
	    // Prn.prn("The difference between the exponentials is more than 300. n1 = "
	    // + this + " and n2 = " + largeNumber2) ;
	...
	if (this.exp > largeNumber2.exp) {
	    m = this.mantissa + largeNumber2.mantissa
		    / Math.pow(10, this.exp - largeNumber2.exp);
	    e = this.exp;
	} else if (this.exp < largeNumber2.exp) {
	    m = this.mantissa / Math.pow(10, largeNumber2.exp - this.exp)
		    + largeNumber2.mantissa;
	    e = largeNumber2.exp;
	} else {
	    m = this.mantissa + largeNumber2.mantissa;
	    e = this.exp;
