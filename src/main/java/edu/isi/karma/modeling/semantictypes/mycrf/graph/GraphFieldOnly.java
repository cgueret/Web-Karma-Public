...
/**
 * This class represents a single node graph. It also stores its computed
 * partition function value, potential, and marginals.
 * 
 * @author amangoel
 * 
 */
public class GraphFieldOnly implements GraphInterface {
    ...
    public GraphFieldOnly(String file, boolean labeled,
	    GlobalDataFieldOnly globalData) {
	...
	if (labeled) {
	    node.labelIndex = globalData.labels
		    .indexOf(tokens[tokens.length - 1]);
	} else {
	    node.labelIndex = -1;
	}
    ...
    public GraphFieldOnly(String fieldString, String label,
	    ArrayList<String> features, GlobalDataFieldOnly globalData) {
	...
	for (int i = 0; i < globalData.labels.size(); i++) {
	    tmp.plusEquals(LargeNumber
		    .makeLargeNumberUsingExponent(potentialExpForLabelIndex(i)));
	}
	Z = tmp;
    }

    public void compute_graphPotential() {
	this.graphPotential = LargeNumber
		.makeLargeNumberUsingExponent(potentialExpForLabelIndex(node.labelIndex));
    }
	...
	for (int f = 0; f < globalData.crfModel.ffs.size(); f++) {
	    ...
	    if (ff.labelIndex == labelIndex
		    && this.node.features.contains(ff.feature)) {
	...
	for (int f = 0; f < globalData.crfModel.ffs.size(); f++) {
	    LblFtrPair ff = globalData.crfModel.ffs.get(f);
	    lhs = (this.node.labelIndex == ff.labelIndex && this.node.features
		    .contains(ff.feature)) ? 1.0 : 0.0;
	    rhs = (node.features.contains(ff.feature)) ? this.nodeMarginals[ff.labelIndex]
		    : 0.0;
	    gradient[f] = lhs - rhs;
	}
    }
