...
/**
 * This class generates features for tokens. Some of the features are:
...
public class RegexFeatureExtractor {
	...
	if (parts.size() == 1) {
	    ...
	    feature_list.add(Feature.ends_with_token_
		    + parts.get(parts.size() - 1).string);
	    return feature_list;
	}
    }
	...
	if (part.type == Type.pure_alpha) {
		...
		if (part.string.charAt(i) >= 'A'
			&& part.string.charAt(i) <= 'Z')
		    continue;
		else {
		    all_caps = false;
		    break;
	    ...
	    else if (first_character.charAt(0) >= 'A'
		    && first_character.charAt(0) <= 'Z')
	    ...
	    feature_list
		    .add(Feature.after_decimal_len_ + decimal_part.length());

	    if (!first_part.equals(""))
		feature_list.add(Feature.starting_digit_
			+ first_part.substring(0, 1));

	    if (!first_part.equals(""))
		feature_list.add(Feature.unit_place_digit_
			+ first_part.substring(first_part.length() - 1));

	    if (!decimal_part.equals(""))
		feature_list.add(Feature.tenth_place_digit_
			+ decimal_part.substring(0, 1));
