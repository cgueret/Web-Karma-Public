...
public class GraphBuilder {
    ...
    public GraphBuilder(OntologyManager ontologyManager,
	    List<SemanticType> semanticTypes,
	    boolean separateDomainInstancesForSameDataProperties) {
	...
	graph = new DirectedWeightedMultigraph<Vertex, LabeledWeightedEdge>(
		LabeledWeightedEdge.class);
	...
	logger.info("total number of nodes in the graph: "
		+ this.graph.vertexSet().size());
	logger.info("total number of links in the graph: "
	...
	for (int i = 0; i < this.semanticTypes.size(); i++) {
	    ...
	    if (nodeType == null) {
		logger.debug("could not find type of " + label
			+ " in the ontology.");
		continue;
	    }

	    Vertex v = new Vertex(id, semanticType.getType(), semanticType,
		    nodeType);
	    semanticNodes.add(v);
	    graph.addVertex(v);
	}
	...
	if (!nodesLabelCounter.containsKey(THING_URI)) {
	    Vertex v = new Vertex(createNodeID(THING_URI), new URI(THING_URI,
		    THING_NS, THING_PREFIX), NodeType.Class);
	...
	for (Vertex v : vertexList) {
	    ...
	    if (domainURI == null || domainURI.getUriString() == null
		    || domainURI.getUriString().trim().length() == 0)
		continue;
		...
		if (visitedDataProperties.indexOf(domainClass
			+ v.getUriString()) != -1
			|| nodesLabelCounter.get(domainClass) == null) {
		    id = createNodeID(domainClass);
		    Vertex domain = new Vertex(id, domainURI, NodeType.Class);
		    graph.addVertex(domain);
		    v.setDomainVertexId(domain.getID());
	...
	// The only place in which we add children is where we are looking for
	// domain class of a property.
	// In this case, in addition to the domain class, we add all of its
	// children too.
	...
	while (recentlyAddedNodes.size() > 0) {
		...
		if (recentlyAddedNodes.get(i).getNodeType() == NodeType.Class) {
		    opDomainClasses = ontologyManager.getDomainsGivenRange(
			    label, true);
		    superClasses = ontologyManager
			    .getSuperClasses(label, false);
		} else if (recentlyAddedNodes.get(i).getNodeType() == NodeType.DataProperty) {
		    dpDomainClasses = ontologyManager.getDomainsGivenProperty(
			    label, true);
		}
		...
		for (int j = 0; j < newAddedClasses.size(); j++) {
		    if (!nodesLabelCounter.containsKey(newAddedClasses.get(j))) { // if
										  // node
										  // is
										  // not
										  // in
			...
			Vertex v = new Vertex(createNodeID(label),
				ontologyManager
					.getURIFromString(newAddedClasses
						.get(j)), NodeType.Class);
			newNodes.add(v);
			this.graph.addVertex(v);
		    }
		}
    ...
    //
    // if
    // (!ontologyManager.isDataProperty(semanticTypes.get(i).getType().trim()))
    // continue;
    //
    ...
    // if (nodesLabelCounter.get(domainClass) == null) {
    // id = createNodeID(domainClass);
    // Vertex v = new Vertex(id, ontologyManager.getNameFromURI(domainClass),
    // NodeType.Class);
    // semanticNodes.add(v);
    // graph.addVertex(v);
    //
	...
	for (int i = 0; i < vertices.length; i++) {
		...
		// create a link from the domain and all its subclasses of this
		// DataProperty to range
		if (target.getNodeType() == NodeType.DataProperty) {
		    ...
		    if (domain != null
			    && domain.trim().equalsIgnoreCase(
				    sourceLabel.trim()))

		    // dataProperties =
		    // ontologyManager.getDataProperties(sourceLabel,
		    // targetLabel, true);
		    // for (int k = 0; k < dataProperties.size(); k++)

		    {
			// label of the data property nodes is equal to name of
			// the data properties
			label = targetLabel; // dataProperties.get(k);
			id = createLinkID(label);
			LabeledWeightedEdge e = new LabeledWeightedEdge(id,
				ontologyManager.getURIFromString(label),
				LinkType.DataProperty);
		...
		// create a link from the domain and all its subclasses of
		// ObjectProperties to range and all its subclasses
		if (target.getNodeType() == NodeType.Class) {
		    objectProperties = ontologyManager.getObjectProperties(
			    sourceLabel, targetLabel, true);

		    for (int k = 0; k < objectProperties.size(); k++) {
			label = objectProperties.get(k);

			List<String> dirDomains = ontologyManager.getOntCache()
				.getPropertyDirectDomains().get(label);
			List<String> dirRanges = ontologyManager.getOntCache()
				.getPropertyDirectRanges().get(label);

			if (dirDomains != null
				&& dirDomains.indexOf(sourceLabel) != -1
				&& dirRanges != null
				&& dirRanges.indexOf(targetLabel) != -1)
			    inherited = false;

			id = createLinkID(label);
			LabeledWeightedEdge e = new LabeledWeightedEdge(id,
				ontologyManager.getURIFromString(label),
				LinkType.ObjectProperty);
			this.graph.addEdge(source, target, e);

			// prefer the links which are actually defined between
			// source and target in ontology over inherited ones.
			if (inherited)
			    this.graph.setEdgeWeight(e, DEFAULT_WEIGHT
				    + MIN_WEIGHT);
			else
		...
		if (target.getNodeType() == NodeType.Class) {
		    ...
		    if (ontologyManager.isSubClass(targetLabel, sourceLabel,
			    false)
			    || ontologyManager.isSuperClass(sourceLabel,
				    targetLabel, false)) {
			id = createLinkID(SUBCLASS_URI);
			LabeledWeightedEdge e = new LabeledWeightedEdge(id,
				new URI(SUBCLASS_URI, SUBCLASS_NS,
					SUBCLASS_PREFIX), LinkType.HasSubClass);
			this.graph.addEdge(source, target, e);
	...
	// logger.info("number of links added to graph: " +
	// this.graph.edgeSet().size());
	logger.debug("exit>");
    }
	...
	for (int i = 0; i < vertices.length; i++) {
		...
		// create a link from all Thing nodes to nodes who don't have
		// any superclasses
		if (target.getNodeType() == NodeType.Class) {
		    id = createLinkID(SUBCLASS_URI);
		    LabeledWeightedEdge e = new LabeledWeightedEdge(
			    id,
	...
	Vertex newNode = new Vertex(id,
		ontologyManager.getURIFromString(label), node.getNodeType());
	...
	LabeledWeightedEdge[] outgoing = graph.outgoingEdgesOf(source).toArray(
		new LabeledWeightedEdge[0]);
	LabeledWeightedEdge[] incoming = graph.incomingEdgesOf(source).toArray(
		new LabeledWeightedEdge[0]);
	...
	if (outgoing != null)
		...
		id = createLinkID(label);
		LabeledWeightedEdge e = new LabeledWeightedEdge(id, new URI(
			outgoing[i].getUriString(), outgoing[i].getNs(),
	...
	if (incoming != null)
		...
		id = createLinkID(label);
		LabeledWeightedEdge e = new LabeledWeightedEdge(id, new URI(
			incoming[i].getUriString(), incoming[i].getNs(),
	...
	s = source;
	t = target;
	List<String> objectProperties = ontologyManager.getObjectProperties(
		s.getUriString(), t.getUriString(), true);
	...
	logger.info("time to add domain of data property nodes to graph: "
		+ elapsedTimeSec);
	...
	// logger.info("time to add links from Thing (root): " +
	// elapsedTimeSec);
