...
public class Alignment {
	...
	public int compare(SemanticType o1, SemanticType o2) {
	    // String s1 = (o1.getDomain() !=
	    // null?o1.getDomain().getUriString():"") +
	    // o1.getType().getUriString();
	    // String s2 = (o2.getDomain() !=
	    // null?o2.getDomain().getUriString():"") +
	    // o2.getType().getUriString();
	...
	graphBuilder = new GraphBuilder(ontologyManager, this.semanticTypes,
		separateDomainInstancesForSameDataProperties);
	...
	LabeledWeightedEdge[] links = linksForcedByUser
		.toArray(new LabeledWeightedEdge[0]);
	for (LabeledWeightedEdge link : links) {
	    if (link.getTarget().getID()
		    .equalsIgnoreCase(e.getTarget().getID()))
		clearUserLink(link.getID());
	}
	linksForcedByUser.add(e);
	logger.info("link " + e.getID()
		+ " has been added to user selected links.");
    }
	...
	LabeledWeightedEdge[] allLinks = this.graphBuilder.getGraph().edgeSet()
		.toArray(new LabeledWeightedEdge[0]);
	...
	for (int j = 0; j < linkIds.size(); j++) {
	    ...
	    if (!found)
		logger.info("link with ID " + linkIds.get(j)
	...
	for (int i = 0; i < linksForcedByUser.size(); i++) {
		...
		logger.info("link " + linkId
			+ " has been removed from  user selected links.");
		align();
		return;
	    }
	}
	...
	for (int j = 0; j < linkIds.size(); j++) {
	    for (int i = 0; i < linksForcedByUser.size(); i++) {
		if (linksForcedByUser.get(i).getID()
			.equalsIgnoreCase(linkIds.get(j))) {
		    linksForcedByUser.remove(i);
		    logger.info("link " + linkIds.get(j)
			    + " has been removed from user selected links.");
		}
	    }
	}
	...
	for (int i = 0; i < allLinks.length; i++) {
		...
		// GraphUtil.printGraph(this.graphBuilder.getGraph());
		addToLinksForcedByUserList(this.graphBuilder.getGraph()
			.getEdge(v, target));

		// do we need to keep the outgoing links of the source which are
		// already in the tree?

		logger.info("domain of the link "
			+ linkId
			+ " has been replicated and graph has been changed successfully.");
		align();
	...
	for (Vertex v : this.steinerTree.vertexSet()) {
		...
		LabeledWeightedEdge[] incomingLinks = this.steinerTree
			.incomingEdgesOf(v).toArray(new LabeledWeightedEdge[0]);
    ...
    public List<LabeledWeightedEdge> getAlternatives(String nodeId,
	    boolean includeAssignedLink) {
	...
	for (Vertex v : this.graphBuilder.getGraph().vertexSet()) {
	    if (v.getID().equalsIgnoreCase(nodeId)) {
		LabeledWeightedEdge[] incomingLinks = this.graphBuilder
			.getGraph().incomingEdgesOf(v)
			.toArray(new LabeledWeightedEdge[0]);
		if (incomingLinks != null && incomingLinks.length > 0) {

		    for (int i = 0; i < incomingLinks.length; i++) {
			if (!includeAssignedLink) {
			    if (assignedLink.getID().equalsIgnoreCase(
				    incomingLinks[i].getID()))
				continue;
			}

			// if the node is not in the UI, don't show it to the
			// user
			// Scenario: multiple domain, then again merge it. The
			// created node is in the graph but not in the tree.
			// if
			// (displayedNodes.indexOf(incomingLinks[i].getSource().getID())
			// == -1)
			// continue;
	...
	// order of adding lists is important: linksPreferredByUI should be
	// first
	...
	for (int i = 0; i < allLinks.length; i++) {
		...
		logger.debug("link " + linkId
			+ " has been added to preferred UI links.");
		return;
	    }
	}
	...
	GraphPreProcess graphPreProcess = new GraphPreProcess(
		this.graphBuilder.getGraph(), semanticNodes, selectedLinks);
	UndirectedGraph<Vertex, LabeledWeightedEdge> undirectedGraph = graphPreProcess
		.getUndirectedGraph();
	...
	WeightedMultigraph<Vertex, LabeledWeightedEdge> tree = steinerTree
		.getSteinerTree();
	if (tree == null) {
	...
	logger.info("total number of nodes in steiner tree: "
		+ this.steinerTree.vertexSet().size());
	logger.info("total number of edges in steiner tree: "
		+ this.steinerTree.edgeSet().size());
    ...
    public void updateLinksDirections(Vertex root, LabeledWeightedEdge e,
	...
	LabeledWeightedEdge[] incomingLinks = treeClone.incomingEdgesOf(root)
		.toArray(new LabeledWeightedEdge[0]);
	if (incomingLinks != null && incomingLinks.length != 0) {
	    for (int i = 0; i < incomingLinks.length; i++) {
		...
		LabeledWeightedEdge inverseLink = new LabeledWeightedEdge(
			inLink.getID(), new URI(inLink.getUriString(),
				inLink.getNs(), inLink.getPrefix()),
			inLink.getLinkType(), true);
	...
	LabeledWeightedEdge[] outgoingLinks = treeClone.outgoingEdgesOf(root)
		.toArray(new LabeledWeightedEdge[0]);
