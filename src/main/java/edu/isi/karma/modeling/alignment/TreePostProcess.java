...
public class TreePostProcess {
	...
	this.tree = (DirectedWeightedMultigraph<Vertex, LabeledWeightedEdge>) GraphUtil
		.asDirectedGraph(tree);
	...
	for (Vertex v : this.tree.vertexSet()) {
	    BreadthFirstIterator<Vertex, LabeledWeightedEdge> i = new BreadthFirstIterator<Vertex, LabeledWeightedEdge>(
		    this.tree, v);
     ...
     * if (root == null) return;
     * 
     * Vertex source, target; LabeledWeightedEdge inLink;
     * 
     * LabeledWeightedEdge[] incomingLinks =
     * this.tree.incomingEdgesOf(root).toArray(new LabeledWeightedEdge[0]); if
     * (incomingLinks != null && incomingLinks.length != 0) { for (int i = 0; i
     * < incomingLinks.length; i++) {
     * 
     * inLink = incomingLinks[i]; source = inLink.getSource(); target =
     * inLink.getTarget();
     * 
     * // don't remove the incoming link from parent to this node if
     * (inLink.getID().equalsIgnoreCase(e.getID())) continue;
     * 
     * LabeledWeightedEdge inverseLink = new LabeledWeightedEdge(inLink.getID(),
     * new Name(inLink.getUri(), inLink.getNs(), inLink.getPrefix()),
     ...
     * 
     * LabeledWeightedEdge[] outgoingLinks =
     * this.tree.outgoingEdgesOf(root).toArray(new LabeledWeightedEdge[0]);
     * 
     * if (outgoingLinks == null || outgoingLinks.length == 0) return;
     * 
     * 
     * for (int i = 0; i < outgoingLinks.length; i++) { target =
     * outgoingLinks[i].getTarget(); updateLinksDirections(target,
     * outgoingLinks[i]); } }
     */
