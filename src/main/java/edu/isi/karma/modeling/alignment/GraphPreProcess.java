...
public class GraphPreProcess {
	...
	gPrime = (DirectedWeightedMultigraph<Vertex, LabeledWeightedEdge>) this.graph
		.clone();
	...
	if (selectedLinks != null)
		...
		if (status == LinkStatus.PreferredByUI) {
		    gPrime.setEdgeWeight(e, GraphBuilder.DEFAULT_WEIGHT
			    - GraphBuilder.MIN_WEIGHT);
		    ...
		    LabeledWeightedEdge[] incomingLinks = gPrime
			    .incomingEdgesOf(target).toArray(
				    new LabeledWeightedEdge[0]);
		    for (LabeledWeightedEdge inLink : incomingLinks) {
			gPrime.removeAllEdges(inLink.getSource(),
				inLink.getTarget());
		    }
	...
	// It is possible that some data property nodes have multiple incoming
	// links from
	// different instances of the same class. We only keep the one that
	// comes from its domain instance.
	for (Vertex v : gPrime.vertexSet()) {
	    ...
	    LabeledWeightedEdge[] incomingLinks = gPrime.incomingEdgesOf(v)
		    .toArray(new LabeledWeightedEdge[0]);
	    if (incomingLinks != null && incomingLinks.length != 0) {

		for (int i = 0; i < incomingLinks.length; i++) {
		    if (!incomingLinks[i].getSource().getID()
			    .equalsIgnoreCase(domainVertexId)) {
			if (incomingLinks.length > 1) // only for data property
						      // nodes who have links
						      // from multiple instances
						      // of the same class
			    gPrime.removeEdge(incomingLinks[i]);
		    } else if (!steinerNodes.contains(incomingLinks[i]
			    .getSource()))
