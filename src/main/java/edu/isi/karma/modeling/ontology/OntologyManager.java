...
public class OntologyManager {
	...
	try {
	    ...
	    // Store the new namespaces information in the namespace map
	    // maintained in OntologyGraphManager
	    // String baseNS = m.getNsPrefixURI("");
	    // m.setNsPrefix("dv" + ontologyNSIndex++, baseNS);

	    // System.out.println("Prefix map:" +
	    // ontologyModel.getNsPrefixMap());
	} catch (Throwable t) {
	...
	if (r == null) {// || !ontModel.containsResource(r)) {
	    logger.debug("Could not find the resource " + uri
    ...
    /**
     * This method gets a resource (class or property) and adds the parents of
     * the resource to the second parameter. If third parameter is set to true,
     * it adds the parents recursively.
     * 
     * @param r
     * @param resources
     * @param recursive
     */
    public void getParents(OntResource r, List<OntResource> resources,
	    boolean recursive) {
    ...
    /**
     * This method gets a resource (class or property) and adds the children of
     * the resource to the second parameter. If third parameter is set to true,
     * it adds the children recursively.
     * 
     * @param r
     * @param resources
     * @param recursive
     */
    public void getChildren(OntResource r, List<OntResource> resources,
	    boolean recursive) {
    ...
    /**
     * This method gets a resource and adds the members of the resource to the
     * second parameter. For example, for a resource like "A or (B and C)", it
     * extracts three elements A, B, C. If third parameter is set to true, it
     * also adds the children of each member.
     * 
     * @param r
     * @param resources
     * @param recursive
     */
    public void getMembers(OntResource r, List<OntResource> resources,
	    boolean recursive) {
	...
	if (r.isClass()) {
		...
		for (Iterator<? extends OntClass> i = ic.listOperands(); i
			.hasNext();) {
		    OntClass op = (OntClass) i.next();
		    getMembers(op, resources, recursive);
		}
    ...
    /**
     * If @param superClassUri is a superclass of @param subClassUri, it returns
     * true; otherwise, false. If third parameter is set to true, it also
     * considers indirect superclasses.
     * 
     * @param superClassUri
     * @param subClassUri
     * @param recursive
     * @return
     */
    public boolean isSuperClass(String superClassUri, String subClassUri,
	    boolean recursive) {
    ...
    /**
     * If @param subClassUri is a subclass of @param superClassUri, it returns
     * true; otherwise, false. If third parameter is set to true, it also
     * considers indirect subclaases.
     * 
     * @param subClassUri
     * @param superClassUri
     * @param recursive
    ...
    /**
     * returns URIs of all subclasses of @param classUri (also considers
     * indirect subclasses if second parameter is true).
     * 
     * @param classUri
     * @param recursive
     * @return
    ...
    /**
     * returns URIs of all superclasses of @param classUri (also considers
     * indirect superclasses if second parameter is true).
     * 
     * @param classUri
     * @param recursive
     * @return
    ...
    /**
     * This method takes a property URI and returns all domains of that
     * property. If @param recursive is true, it also returns the children of
     * the domains.
     * 
     * @param propertyUri
     * @param recursive
     * @return
     */
    public List<String> getDomainsGivenProperty(String propertyUri,
	    boolean recursive) {
    ...
    /**
     * This method takes @param rangeClassUri and for object properties whose
     * ranges includes this parameter, returns all of their domains. If @param
     * recursive is true, it also returns the children of the domains.
     * 
     * @param rangeClassUri
     * @param recursive
     * @return
     */
    public List<String> getDomainsGivenRange(String rangeClassUri,
	    boolean recursive) {

	List<String> objectProperties = ontCache.getDirectInObjectProperties()
		.get(rangeClassUri);
	...
	for (int i = 0; i < objectProperties.size(); i++) {
	    if (!recursive)
		temp = ontCache.getPropertyDirectDomains().get(
			objectProperties.get(i));
	    else
		temp = ontCache.getPropertyIndirectDomains().get(
			objectProperties.get(i));
	    if (temp != null)
		domains.addAll(temp);
	}
    ...
    /**
     * This function takes a class and a data property and says if the class is
     * in domain of that data property or not. If @param includeinheritance is
     * true, it also returns the data properties inherited from parents, for
     * example, if A is superclass of B, and we have a datatype property P from
     * A to xsd:int, then calling this function with (B, P, false) returns
     * nothing, but calling with (B, P, true) returns the property P.
     * 
     ...
     * @return
     */
    public List<String> getDataProperties(String domainClassUri,
	    String propertyUri, boolean inheritance) {

	List<String> propertyDomains;
	List<String> results = new ArrayList<String>();

	if (!inheritance)
	    propertyDomains = ontCache.getPropertyDirectDomains().get(
		    propertyUri);
	else
	    propertyDomains = ontCache.getPropertyIndirectDomains().get(
		    propertyUri);

	if (propertyDomains != null
		&& propertyDomains.indexOf(domainClassUri) != -1)
    ...
    /**
     * This method extracts all the object properties between two classes
     * (object properties who have @param domainClassUri in their domain and @param
     * rangeClassUri in their range). If @param inheritance is true, it also
     * returns the object properties inherited from parents.
     * 
     * @param domainClassUri
     * @param rangeClassUri
     * @param inheritance
     * @return
     */
    public List<String> getObjectProperties(String domainClassUri,
	    String rangeClassUri, boolean inheritance) {

	List<String> results;

	if (!inheritance)
	    results = ontCache.getDirectDomainRangeProperties().get(
		    domainClassUri + rangeClassUri);
	else
	    results = ontCache.getIndirectDomainRangeProperties().get(
		    domainClassUri + rangeClassUri);
    ...
    /**
     * This function takes a class uri and returns the datatype properties who
     * have this class in their domain. If second parameter set to True, it also
     * returns the datatype properties inherited from parents of the given
     * class.
     * 
     * @param domainClassUri
     * @param inheritance
     * @return
     */
    public List<String> getDataPropertiesOfClass(String domainClassUri,
	    boolean inheritance) {
	...
	if (!inheritance)
	    ...
	    results = ontCache.getIndirectOutDataProperties().get(
		    domainClassUri);
    ...
    /**
     * This function takes a class uri and returns the object properties who
     * have this class in their domain. If second parameter set to True, it also
     * returns the object properties inherited from parents of the given class.
     * 
     * @param domainClassUri
     * @param inheritance
     * @return
     */
    public List<String> getObjectPropertiesOfClass(String domainClassUri,
	    boolean inheritance) {

	List<String> results;

	if (!inheritance)
	    results = ontCache.getDirectOutObjectProperties().get(
		    domainClassUri);
	else
	    results = ontCache.getIndirectOutObjectProperties().get(
		    domainClassUri);
