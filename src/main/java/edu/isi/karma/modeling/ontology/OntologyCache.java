...
public class OntologyCache {
	...
	// ExtendedIterator<OntProperty> itrOP =
	// ontologyManager.getOntModel().listAllOntProperties();
	ExtendedIterator<ObjectProperty> itrOP = ontologyManager.getOntModel()
	...
	while (itrOP.hasNext()) {
	    ...
	    ExtendedIterator<? extends OntResource> itrDomains = op
		    .listDomain();
	    ...
	    if (temp == null)
		propertyDirectDomains.put(op.getURI(),
			ontologyManager.getResourcesURIs(directDomains));
	    else
		temp.addAll(ontologyManager.getResourcesURIs(directDomains));

	    for (int i = 0; i < directDomains.size(); i++) {
		temp = directOutObjectProperties.get(directDomains.get(i)
			.getURI());
		if (temp == null) {
		    temp = new ArrayList<String>();
		    directOutObjectProperties.put(
			    directDomains.get(i).getURI(), temp);
		}
		temp.add(op.getURI());
	    }
		...
		ontologyManager.getChildren(directDomains.get(i), allDomains,
			true);
	    }

	    temp = propertyIndirectDomains.get(op.getURI());
	    if (temp == null)
		propertyIndirectDomains.put(op.getURI(),
			ontologyManager.getResourcesURIs(allDomains));
	    else
		temp.addAll(ontologyManager.getResourcesURIs(allDomains));

	    for (int i = 0; i < allDomains.size(); i++) {
		temp = indirectOutObjectProperties.get(allDomains.get(i)
			.getURI());
		if (temp == null) {
		    temp = new ArrayList<String>();
		    indirectOutObjectProperties.put(allDomains.get(i).getURI(),
			    temp);
		}
		temp.add(op.getURI());
	    }
	    ...
	    if (temp == null)
		propertyDirectRanges.put(op.getURI(),
			ontologyManager.getResourcesURIs(directRanges));
	    else
		temp.addAll(ontologyManager.getResourcesURIs(directRanges));

	    for (int i = 0; i < directRanges.size(); i++) {
		temp = directInObjectProperties.get(directRanges.get(i)
			.getURI());
		if (temp == null) {
		    temp = new ArrayList<String>();
		    directInObjectProperties.put(directRanges.get(i).getURI(),
			    temp);
		}
		temp.add(op.getURI());
	    }
		...
		ontologyManager.getChildren(directRanges.get(i), allRanges,
			true);
	    }

	    temp = propertyIndirectRanges.get(op.getURI());
	    if (temp == null)
		propertyIndirectRanges.put(op.getURI(),
			ontologyManager.getResourcesURIs(allRanges));
	    else
		temp.addAll(ontologyManager.getResourcesURIs(allRanges));

	    for (int i = 0; i < allRanges.size(); i++) {
		temp = indirectInObjectProperties
			.get(allRanges.get(i).getURI());
		if (temp == null) {
		    temp = new ArrayList<String>();
		    indirectInObjectProperties.put(allRanges.get(i).getURI(),
			    temp);
		}
		temp.add(op.getURI());
	    }
		...
		for (int j = 0; j < directRanges.size(); j++) {
		    temp = directDomainRangeProperties.get(directDomains.get(i)
			    .toString() + directRanges.get(j).toString());
		    if (temp == null) {
			temp = new ArrayList<String>();
			directDomainRangeProperties.put(directDomains.get(i)
				.toString() + directRanges.get(j).toString(),
				temp);
		    }
		...
		for (int j = 0; j < allRanges.size(); j++) {
		    temp = indirectDomainRangeProperties.get(allDomains.get(i)
			    .toString() + allRanges.get(j).toString());
		    if (temp == null) {
			temp = new ArrayList<String>();
			indirectDomainRangeProperties
				.put(allDomains.get(i).toString()
					+ allRanges.get(j).toString(), temp);
		    }
		    if (temp.indexOf(op.getURI()) == -1)
			temp.add(op.getURI());
		}
	...
	// ExtendedIterator<OntProperty> itrDP =
	// ontologyManager.getOntModel().listAllOntProperties();
	ExtendedIterator<DatatypeProperty> itrDP = ontologyManager
		.getOntModel().listDatatypeProperties();
	OntResource d;
	OntResource r;
	...
	while (itrDP.hasNext()) {
	    ...
	    // getting domains and subclasses
	    ExtendedIterator<? extends OntResource> itrDomains = dp
		    .listDomain();
	    while (itrDomains.hasNext()) {
		d = itrDomains.next();
		ontologyManager.getMembers(d, directDomains, false);
	    ...
	    if (temp == null)
		propertyDirectDomains.put(dp.getURI(),
			ontologyManager.getResourcesURIs(directDomains));
	    else
		temp.addAll(ontologyManager.getResourcesURIs(directDomains));

	    for (int i = 0; i < directDomains.size(); i++) {
		temp = directOutDataProperties.get(directDomains.get(i)
			.getURI());
		if (temp == null) {
		    temp = new ArrayList<String>();
		    directOutDataProperties.put(directDomains.get(i).getURI(),
			    temp);
		}
		temp.add(dp.getURI());
	    }
	    ...
	    if (temp == null)
		propertyIndirectDomains.put(dp.getURI(),
			ontologyManager.getResourcesURIs(allDomains));
	    else
		temp.addAll(ontologyManager.getResourcesURIs(allDomains));

	    for (int i = 0; i < allDomains.size(); i++) {
		temp = indirectOutDataProperties
			.get(allDomains.get(i).getURI());
		if (temp == null) {
		    temp = new ArrayList<String>();
		    indirectOutDataProperties.put(allDomains.get(i).getURI(),
			    temp);
		}
		temp.add(dp.getURI());
	    }
	    ...
	    if (temp == null)
		propertyDirectRanges.put(dp.getURI(),
			ontologyManager.getResourcesURIs(directRanges));
	    else
		temp.addAll(ontologyManager.getResourcesURIs(directRanges));
	    ...
	    if (temp == null)
		propertyIndirectRanges.put(dp.getURI(),
			ontologyManager.getResourcesURIs(allRanges));
	    else
		temp.addAll(ontologyManager.getResourcesURIs(allRanges));
	...
	ontologyManager.getOntModel().setNsPrefix("rdfs",
    ...
    /**
     * If the inheritance is true, it adds all the sub-classes of the domain and
     * range of super-properties too.
     * 
     * @param inheritance
     */
	...
	for (String p : propertyDirectDomains.keySet()) {
	    ...
	    List<String> superProperties = ontologyManager.getSuperProperties(
		    p, true);
