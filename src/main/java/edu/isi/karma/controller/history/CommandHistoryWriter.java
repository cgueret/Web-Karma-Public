...
public class CommandHistoryWriter {
    ...
    public CommandHistoryWriter(ArrayList<Command> history,
	...
	for (Command command : history) {
		...
		String vWorksheetId = HistoryJsonUtil.getStringValue(
			HistoryArguments.vWorksheetId.name(), json);
	...
	for (String wkName : comMap.keySet()) {
		...
		commObj.put(HistoryArguments.commandName.name(),
			comm.getCommandName());
		...
		for (int i = 0; i < inputArr.length(); i++) {
		    ...
		    /***
		     * Check the input parameter type and accordingly make
		     * changes
		     ***/
		    if (HistoryJsonUtil.getParameterType(inpP) == ParameterType.hNodeId) {
			String hNodeId = inpP.getString(ClientJsonKeys.value
				.name());
			HNode node = vWorkspace.getRepFactory().getHNode(
				hNodeId);
			JSONArray hNodeRepresentation = node
				.getJSONArrayRepresentation(vWorkspace
					.getRepFactory());
			inpP.put(ClientJsonKeys.value.name(),
				hNodeRepresentation);
	    ...
	    JSONUtil.writeJsonFile(commArr, HistoryJsonUtil
		    .constructWorksheetHistoryJsonFilePath(wkName,
			    vWorkspace.getPreferencesId()));
	}
    }

    private boolean commandAlreadyexists(JSONArray commArr, JSONObject commObj1)
