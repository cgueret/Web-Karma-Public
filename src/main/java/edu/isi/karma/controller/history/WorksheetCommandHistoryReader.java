...
public class WorksheetCommandHistoryReader {
    ...
    private static Logger logger = LoggerFactory
	    .getLogger(WorksheetCommandHistoryReader.class);

    public WorksheetCommandHistoryReader(String vWorksheetId,
    ...
    public void readAndExecuteCommands(CommandTag tag)
	    throws FileNotFoundException, JSONException, KarmaException,
	    CommandException {
	String worksheetName = vWorkspace.getViewFactory()
		.getVWorksheet(vWorksheetId).getWorksheet().getTitle();
	File historyFile = new File(
		HistoryJsonUtil.constructWorksheetHistoryJsonFilePath(
			worksheetName, vWorkspace.getPreferencesId()));
	JSONArray historyJson = (JSONArray) JSONUtil.createJson(new FileReader(
		historyFile));

	ExecutionController ctrl = WorkspaceRegistry.getInstance()
		.getExecutionController(vWorkspace.getWorkspace().getId());
	HashMap<String, CommandFactory> commandFactoryMap = ctrl
		.getCommandFactoryMap();

	for (int i = 0; i < historyJson.length(); i++) {
	    JSONObject commObject = (JSONObject) historyJson.get(i);
	    JSONArray tags = commObject.getJSONArray(HistoryArguments.tags
		    .name());
    ...
    public void executeListOfCommands(List<String> commandsJsonList)
	    throws JSONException, KarmaException, CommandException {
	ExecutionController ctrl = WorkspaceRegistry.getInstance()
		.getExecutionController(vWorkspace.getWorkspace().getId());
	HashMap<String, CommandFactory> commandFactoryMap = ctrl
    ...
    private void executeCommand(JSONObject commObject,
	    ...
	    throws JSONException, KarmaException, CommandException {
	JSONArray inputParamArr = (JSONArray) commObject
		.get(HistoryArguments.inputParameters.name());

	logger.info("Command in history: "
		+ commObject.get(HistoryArguments.commandName.name()));
	// Change the hNode ids, vworksheet id to point to the current worksheet
	// ids
	if (normalizeJsonInput(inputParamArr)) {
	    // Invoke the command
	    CommandFactory cf = commandFactoryMap.get(commObject
		    .get(HistoryArguments.commandName.name()));
	    if (cf != null && cf instanceof JSONInputCommandFactory) {
		logger.info("Executing command from history: "
			+ commObject.get(HistoryArguments.commandName.name()));
		JSONInputCommandFactory scf = (JSONInputCommandFactory) cf;
		Command comm = scf.createCommand(inputParamArr, vWorkspace);
		if (comm != null) {
		    logger.info("Executing command: "
			    + commObject.get(HistoryArguments.commandName
				    .name()));
		    vWorkspace.getWorkspace().getCommandHistory()
			    .doCommand(comm, vWorkspace);
		} else
	...
	HTable hTable = vWorkspace.getViewFactory().getVWorksheet(vWorksheetId)
	...
	for (int i = 0; i < inputArr.length(); i++) {
		...
		JSONArray hNodeJSONRep = new JSONArray(
			inpP.getString(ClientJsonKeys.value.name()));
		for (int j = 0; j < hNodeJSONRep.length(); j++) {
		    ...
		    logger.debug("Column being normalized: "
			    + cNameObj.getString("columnName"));
		    HNode node = hTable.getHNodeFromColumnName(cNameObj
			...
			if (j == hNodeJSONRep.length() - 1) { // Found!
			    ...
			    hTable = vWorkspace.getViewFactory()
	...
	try {
	    JSONArray historyJson = (JSONArray) JSONUtil
		    .createJson(new FileReader(historyFile));
