...
public class PublishDatabaseCommand extends Command {
    ...
    protected PublishDatabaseCommand(String id, String vWorksheetId,
	    String dbType, String hostName, String port, String dbName,
	...
	// for now don't save a nested table (we may want to save it as multiple
	// tables with foreign keys)
	if (worksheet.getHeaders().hasNestedTables()) {
	    return new UpdateContainer(new ErrorUpdate(
		    "Saving of nested tables not supported!"));
	}

	Connection conn = null;
	try {
	    conn = dbUtil.getConnection(hostName, Integer.valueOf(port)
		    .intValue(), userName, password, dbName);
	    ...
	    // ONLY columns with given semantic types will be saved; unassigned
	    // columns are ignored
	    if (colNames.isEmpty()) {
		// no columns were modeled
		return new UpdateContainer(new ErrorUpdate(
			"Please align the worksheet before saving."));
	    }
		...
		if (!overwrite && !insert) {
		    ...
		    numRowsNotInserted = insertInTable(worksheet, tableName,
			    colNamesMap, conn);
		} else if (insert) {
     ...
     * @throws SQLException
     */
    private void createTable(String tableName, Collection<String> colNames,
     ...
     * @param tableName
     * @param colNamesMap
     *            key=hNodeId; val=column name (same as semantic type for this
     *            column)
     * @param conn
     * @return number of rows not inserted. A row is not inserted if we have a
     *         type mismatch.
     * @throws SQLException
     */
    private int insertInTable(Worksheet w, String tableName,
	    Map<String, String> colNamesMap, Connection conn)
	    throws SQLException {
	int numOfRowsNotInserted = 0;
	// get col names for existing table
	// some databases are case sensitive when referring to column/table
	// names, so we have
	// to use the "real" case in the queries
	// I don't do this check for now; not sure for which DBs it is required
	ArrayList<String> existingColNames = dbUtil.getColumnNames(tableName,
		conn);
	ArrayList<String> existingColTypes = dbUtil.getColumnTypes(tableName,
		conn);
	// add in the insert only values for these columns; other columns do not
	...
	for (Map.Entry<String, String> colNameInWorksheet : colNamesMap
		.entrySet()) {
		...
		addTheseTypes.put(colNameInWorksheet.getKey(),
			existingColTypes.get(ind));
	    }
	}
	...
	for (Row r : rows) {
	    // insert one row
	    String insertRow = insertInTableRow(r, tableName, addTheseColumns,
		    addTheseTypes);
	    // returns null if that particular row could not be inserted
	    // because there is a number column for which the values are not
	    // numbers
	    if (insertRow != null)
	...
	for (Map.Entry<String, Node> node : r.getNodesMap().entrySet()) {
		...
		if (isDbTypeString(colTypesMap.get(hNodeId))) {
			...
			try {
			    ...
			    logger.error("Row not inserted:"
				    + val
				    + " is not a number as required by the database");
			    return null;
			}
		    }
		}
	...
	String insertQ = "insert into " + tableName + "(" + colNames
		+ ") values (" + colValues + ")";
    ...
    /**
     * Returns a map where key = hnodeId; value=the semantic type used as the
     * column name.
     * 
     * @param w
     * @return a map where key = hnodeId; value=the semantic type used as the
     *         column name <br>
     *         If multiple sem types with same name, use an index to distinguish
     *         them
     */
	...
	try {
	    ...
	    // although we save these we don't reload them (I just left them
	    // here in case we want to in the future)
