...
public class SetSemanticTypeCommandFactory extends CommandFactory implements
	...
	try {
		...
		URI typeName = ontMgr.getURIFromString(type
			.getString(ClientJsonKeys.FullType.name()));
		if (typeName == null) {
		    logger.error("Could not find the resource "
			    + type.getString(ClientJsonKeys.FullType.name())
			    + " in ontology model!");
		    return null;
		}
		URI domainName = null;
		if (type.getString(ClientJsonKeys.Domain.name()) != "")
		    domainName = ontMgr.getURIFromString(type
			    .getString(ClientJsonKeys.Domain.name()));
	...
	String arrStr = HistoryJsonUtil.getStringValue(
		Arguments.SemanticTypesArray.name(), inputJson);
	boolean isPartOfKey = HistoryJsonUtil.getBooleanValue(
		Arguments.isKey.name(), inputJson);
	boolean train = HistoryJsonUtil.getBooleanValue(
		Arguments.trainAndShowUpdates.name(), inputJson);
	...
	try {
		...
		if (typeName == null) {
		    logger.error("Could not find the resource "
			    + type.getString(SemanticTypesUpdate.JsonKeys.FullType
				    .name()) + " in ontology model!");
		    return null;
		}
		URI domainName = null;
	...
	SetSemanticTypeCommand comm = new SetSemanticTypeCommand(
		getNewId(vWorkspace), vWorksheetId, hNodeId, isPartOfKey,
		primaryType, synTypes, train);

	// Change the train flag, so that it does not train while reading from
	// history
	HistoryJsonUtil.setArgumentValue(Arguments.trainAndShowUpdates.name(),
		false, inputJson);
