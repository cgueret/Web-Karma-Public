...
public class ImportDatabaseTableCommand extends CommandWithPreview {
    ...
    // LoggerFactory.getLogger(ImportDatabaseTableCommand.class);
    ...
    public void setRequestedInteractionType(
	...
	if (requestedInteractionType == InteractionType.getPreferencesValues) {
	    c.add(new FetchPreferencesUpdate(vWorkspace,
		    ImportDatabaseTableCommand.class.getSimpleName()
			    + "Preferences"));
	    return c;
	}
	...
	case generateTableList: {
	    ...
	    c.add(new DatabaseTablesListUpdate(dbType, hostname, portnumber,
		    username, password, dBorSIDName, id));
	    return c;
	}
	case previewTable: {
	    c.add(new DatabaseTablePreviewUpdate(dbType, hostname, portnumber,
		    username, password, request.getParameter("tableName"),
		    dBorSIDName, id));
	    return c;
	}
	case importTable: {
	...
	UpdateContainer c = new UpdateContainer();
	try {
	    DatabaseTableImport dbTableImport = new DatabaseTableImport(dbType,
		    hostname, portnumber, username, password, dBorSIDName,
	    ...
	    // Create a new Database Import Command. The interface allows the
	    // user to import
	    // multiple tables
	    ImportDatabaseTableCommand comm = new ImportDatabaseTableCommand(
		    vWorkspace.getRepFactory().getNewId("C"), dbType.name(),
		    hostname, portnumber, username, password, dBorSIDName,
		    vWorkspace);
	    vWorkspace.getWorkspace().getCommandHistory()
		    .setCurrentCommand(comm);
