...
public class PublishRDFCommand extends Command {
    ...
    protected PublishRDFCommand(String id, String vWorksheetId,
	    ...
	    String addInverseProperties, String saveToStore, String hostName,
	    String dbName, String userName, String password, String modelName) {
	super(id);
	...
	final String rdfFileName = ServletContextParameterMap
	...
	try {
		...
		SourceDescription desc = new SourceDescription(
			...
			rdfSourcePrefix, Boolean.valueOf(addInverseProperties),
			false);
		...
		if (worksheet.getHeaders().hasNestedTables()) {
		    ...
		    // System.out.println("RDF start="+
		    // Calendar.getInstance().getTimeInMillis());
		    wrg.generateTriplesRow(worksheet, true);
		    // System.out.println("RDF end="+
		    // Calendar.getInstance().getTimeInMillis());
		}
		...
		if (saveToStore) {
		    // take the contents of the RDF file and save them to the
		    // store
		    logger.info("Using Jena DB:" + hostName + "/" + dbName
			    + " user=" + userName);
		...
		public void generateJson(String prefix, PrintWriter pw,
			...
			outputObject.put(JsonKeys.updateType.name(),
				"PublishRDFUpdate");
			outputObject.put(JsonKeys.fileUrl.name(), "RDF/"
				+ vWorkspace.getPreferencesId() + vWorksheetId
				+ ".n3");
	...
	try {
	    ...
	    prefObject.put(PreferencesKeys.addInverseProperties.name(),
    ...
    private void saveToStore(String rdfFileName) throws ClassNotFoundException,
	    IOException {
	...
	IDBConnection conn = new DBConnection(dbUrl, userName, password,
		"MySQL");
