...
public class PublishWorksheetHistoryCommand extends Command {
	...
	if (!HistoryJsonUtil.historyExists(wkName, wsPreferenceId)) {
	    return new UpdateContainer(new ErrorUpdate(
		    "No history exists for the worksheet!"));
	}

	// Copy the history file to a location accessible from web
	// File existingHistoryFile = new
	// File(HistoryJsonUtil.constructWorksheetHistoryJsonFilePath(wkName,
	// wsPreferenceId));
	// final File newFileName = new File("./src/main/webapp/History/"
	// + HistoryJsonUtil.constructWorksheetHistoryJsonFileName(wkName,
	// wsPreferenceId));
	// try {
	// FileUtil.copyFiles(newFileName, existingHistoryFile);
	// } catch (Exception e1) {
	// logger.error("Error occured while copying the history file to server!",
	// e1);
	// return new UpdateContainer(new
	// ErrorUpdate("Error occured while publishing history for worksheet!"));
	// }
	...
	return new UpdateContainer(new AbstractUpdate() {
		...
		try {
		    ...
		    outputObject.put(
			    JsonKeys.fileUrl.name(),
			    "publish/History/"
				    + HistoryJsonUtil
					    .constructWorksheetHistoryJsonFileName(
						    wkName, wsPreferenceId));
