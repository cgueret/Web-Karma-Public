...
public class GenerateCleaningRulesCommand extends WorksheetCommand {
    ...
    public GenerateCleaningRulesCommand(String id, String worksheetId,
	    String hNodeId, String examples) {
	...
	/*
	 * System.out.println(""+x); Vector<Double> scores =
	 * UtilTools.getScores(x, trainPath);
	 * System.out.println("Scores: "+scores); Vector<Integer> ins
	 * =UtilTools.topKindexs(scores, k); System.out.println("Indexs: "+ins);
	 */
	...
	for (Node node : nodes) {
	    ...
	    String originalVal = "<_START>" + node.getValue().asString()
		    + "<_END>";
	...
	while (iterNum < 10 && !results) // try to find any rule during 5 times
					 // running
	{
	...
	return new UpdateContainer(new CleaningResultUpdate(hNodeId, jsons,
		js2tps));
    }
