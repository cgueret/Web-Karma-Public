...
public class SVGAlignmentUpdate_ForceKarmaLayout extends AbstractUpdate {
    ...
    public SVGAlignmentUpdate_ForceKarmaLayout(String vWorksheetId,
	    String alignmentId, Alignment alignment, List<String> hNodeIdList) {
	super();
	...
	try {
	    ...
	    DirectedWeightedMultigraph<Vertex, LabeledWeightedEdge> treeClone = (DirectedWeightedMultigraph<Vertex, LabeledWeightedEdge>) tree
		    .clone();
		...
		for (Vertex vertex : vertices) {
		    ...
		    vertObj.put(JsonKeys.nodeType.name(), vertex.getNodeType()
			    .name());

		    List<Vertex> nodesWithSemTypesCovered = new ArrayList<Vertex>();
		    int height = getHeight(vertex, nodesWithSemTypesCovered,
			    treeClone);
			...
			hNodeIdsCoveredByVertex.put(v.getSemanticType()
				.getHNodeId());
		    vertObj.put(JsonKeys.hNodesCovered.name(),
			...
			if (vertex.getNodeType() == NodeType.Class) {
			    if (type.isPartOfKey()) {
				vertObj.put(JsonKeys.label.name(),
					vertex.getLocalID() + "*");
			    }
			    ...
			    vertObj_holder.put(JsonKeys.label.name(),
				    JsonValues.key.name());
			    vertObj_holder.put(JsonKeys.id.name(),
				    vertex.getID() + "_holder");
			    vertObj_holder.put(JsonKeys.nodeType.name(),
				    NodeType.DataProperty.name());
			    vertObj_holder.put(JsonKeys.hNodeId.name(),
				    type.getHNodeId());

			    JSONArray hNodeIdsCoveredByVertex_holder = new JSONArray();
			    hNodeIdsCoveredByVertex_holder.put(vertex
				    .getSemanticType().getHNodeId());
			    vertObj_holder.put(JsonKeys.hNodesCovered.name(),
			    ...
			    linkObj.put(JsonKeys.source.name(),
				    nodesIndexcounter);
			    linkObj.put(JsonKeys.target.name(),
				    nodesIndexcounter - 1);
			    linkObj.put(JsonKeys.label.name(),
				    JsonValues.key.name());
		...
		for (LabeledWeightedEdge edge : edges) {
		    ...
		    Set<LabeledWeightedEdge> outEdges = tree
		    ...
		    linkObj.put(JsonKeys.linkStatus.name(), edge
			    .getLinkStatus().name());
			...
			linkObj.put(JsonKeys.linkType.name(),
				JsonValues.objPropertyLink.name());
		    }
		}
	    }
		...
		vertObj.put(JsonKeys.nodeType.name(),
			JsonValues.Unassigned.name());
		vertArr.put(vertObj);
    ...
    private int getHeight(Vertex vertex, List<Vertex> nodesWithSemTypesCovered,
	    DirectedWeightedMultigraph<Vertex, LabeledWeightedEdge> treeClone) {
	BreadthFirstIterator<Vertex, LabeledWeightedEdge> itr = new BreadthFirstIterator<Vertex, LabeledWeightedEdge>(
		treeClone, vertex);
	...
	if (lastNodeWithSemanticType != null) {
	    height = new DijkstraShortestPath<Vertex, LabeledWeightedEdge>(
		    treeClone, vertex, lastNodeWithSemanticType)
		    .getPathEdgeList().size();
	    if (lastNodeWithSemanticType.getNodeType() == NodeType.Class
		    && lastNodeWithSemanticType.getSemanticType() != null
		    && vertex != lastNodeWithSemanticType) {
		height += getHeight(lastNodeWithSemanticType,
			new ArrayList<Vertex>(), treeClone);
	    }

	    if (vertex == lastNodeWithSemanticType
		    && vertex.getNodeType() == NodeType.Class
		    && vertex.getSemanticType() != null) {
		height = 1;
	    }
