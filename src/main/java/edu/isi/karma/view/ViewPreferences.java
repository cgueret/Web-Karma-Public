...
public class ViewPreferences {
    ...
    private static Logger logger = LoggerFactory
	    .getLogger(ViewPreferences.class.getSimpleName());
	...
	public int getIntDefaultValue() {
	    switch (this) {
	    case maxCharactersInHeader:
		return 10;
	    case maxCharactersInCell:
		return 100;
	    case maxRowsToShowInNestedTables:
		return 5;
	    case defaultRowsToShowInTopTables:
		return 10;
	    }
	    return -1;
	}
	...
	try {
		...
		json = (JSONObject) JSONUtil
			.createJson(new FileReader(jsonFile));
		if (json == null) {
		...
		// Create a new JSON preference file using the template
		// preferences file
		createNewPreferencesFileFromTemplate();
	    }
	...
	try {
	    ...
	    try {
		if (json.getJSONObject("ViewPreferences").optJSONObject(
			pref.name()) == null) {
		...
		logger.error(
			"Error occured while adding a new key to the preferences JSON object!",
			e1);
	    }
	}
	return -1;
	...
	try {
	    ...
	    // Check if the command already exists. In that case, we overwrite
	    // the values
	    for (int i = 0; i < commArray.length(); i++) {
		JSONObject obj = commArray.getJSONObject(i);
		if (obj.getString("Command").equals(commandName)) {
    ...
    public void setIntViewPreferenceValue(ViewPreference pref, int value) {
	try {
	    ...
	    logger.error("Error writing the changed preferences to file!"
		    + jsonFile.getName(), e);
	}
    }
}
