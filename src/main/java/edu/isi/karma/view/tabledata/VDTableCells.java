...
public class VDTableCells {
	...
	CellValue cellValue = c.getNode() == null ? null : c.getNode()
		.getValue();
	String valueString = (cellValue == null || cellValue.asString() == null) ? ""
		: cellValue.asString();

	if (valueString.length() > vWorkspace.getPreferences()
	...
	// Add the node id
	if (c.getNode() != null) {
	    jw.key(JsonKeys.nodeId.name()).value(c.getNode().getId());
	}

	// Add the full value if the display value was truncated
	jw.key(isTruncated.name()).value(isValueTruncated);
	if (isValueTruncated) {
	    jw.key(fullValue.name()).value(cellValue.asString());
	}
