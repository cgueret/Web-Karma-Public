...
public class Border {
	...
	jw.object()
		...
		//
		.key(topBorder.name())
		.value(encodeBorder(topStroke,
			css.getCssTag(margin.getHTableId(), margin.getDepth())))
		//
		.key(leftBorder.name())
		.value(encodeBorder(leftStroke,
			css.getCssTag(stroke.getHTableId(), margin.getDepth())))
		//
		.key(rightBorder.name())
		.value(encodeBorder(rightStroke,
			css.getCssTag(stroke.getHTableId(), margin.getDepth())))//
	;
	jw.endObject();
    }
