...
public class GrammarParseTree {
	...
	try {
	    ...
	    FileInputStream file = new FileInputStream(dirpathString
		    + "grammar/MOVgrammar.txt");
	...
	try {
		...
		Vector<TNode> bef = (Vector<TNode>) ba.get(i).getBefore()
			.clone();
    ...
    public static HashMap<String, ArrayList<ArrayList<String>>> deepclone(
	    HashMap<String, ArrayList<ArrayList<String>>> x) {
    ...
    // if the non-terminal has no children, delete the non-terminal from the
    // grammar space
	...
	try {
	    ...
	    System.out.println("toString error: " + x.name + " "
		    + x.children.toString());
	    return "";
	}
    }
	...
	// using the rule to generate a possible parse tree comply with the
	// examples
	if (name == null)
	    return;
	if (name.length() == 0)
	...
	if (rules.size() > 1 || subtemples.containsKey(name)) // more than one
							      // alternatives
	{
		...
		GrammarTreeNode t = new GrammarTreeNode(name);
		/*
		 * if(name.compareTo("swherequantifier")==0) { if(index==0) {
		 * cnt1++; } else if(index==1) { cnt2++; } else { cnt3++; }
		 * System
		 * .out.println("count number>>>"+rules.size()+"="+cnt1+":"
		 * +cnt2+":"+cnt3); }
		 */
		t.parent = par;
		par.addChild(t);
    ...
    /*
     * eos is the set of edit operation of multiple examples p is the original
     * token sequence.
     */
    public boolean initalSubtemplete(Vector<HashSet<String>> sequen,
	    HashMap<String, GrammarTreeNode> globalTemp) {
	diagDesc = sequen;
	// if subtemple exists, we should remove the production rules.
	for (String s : sequen.get(0)) {
	    // System.out.println("etokenspec: "+s);
	    GrammarTreeNode root = NonterminalValidator.getTemplate(
		    "etokenspec", s, globalTemp);
	...
	for (String s : sequen.get(3)) {
	    ...
	    GrammarTreeNode root = NonterminalValidator.getTemplate("qnum", s,
		    globalTemp);
	...
	for (String s : sequen.get(4)) {
	    ...
	    GrammarTreeNode root = NonterminalValidator.getTemplate("snum", s,
		    globalTemp);
	...
	for (String s : sequen.get(5)) {
	    ...
	    GrammarTreeNode root = NonterminalValidator.getTemplate("tnum", s,
		    globalTemp);
	...
	for (String s : sequen.get(6)) {
	    ...
	    GrammarTreeNode root = NonterminalValidator.getTemplate("dnum", s,
		    globalTemp);
	...
	if (subtemples.containsKey(key)) {
	    ...
	    if (size == 1) {
		System.out
			.println("Error occured, should be more than 1 element");
		System.exit(1);
	    }
	    Random r = new Random();
