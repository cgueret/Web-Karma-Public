...
public class RuleGenerator {
	...
	try {
	    ...
	    CommonTreeNodeStream nodes = new CommonTreeNodeStream(parser.rule()
		    .getTree());
    ...
    // replace the tar with a set of new values. each of this new value become a
    // new rule
    ...
    public Vector<String> replacetokspec(Collection<String> whats,
	    Collection<TNode> examplex, int spos, int epos) {
	...
	for (String s : whats) {
		...
		if (tokens[i].compareTo("TOKEN") == 0) {
		    ...
		    if (tokens[i].compareTo(example.get(spos + i).getType()) != 0
			    && tokens[i].compareTo(TNode.ANYTOK) != 0) {
			valid = false;
			break;
		    }
		}
    ...
    public void addElement(ArrayList<ArrayList<String>> s, String name,
	    boolean isoptional) {
	if (s.size() == 0) {
	    if (isoptional) {
    ...
    private double alpha = 2; // the ratio that the terminal should be bigger
			      // than nonterms
    ...
    public double transProb(HashMap<String, Integer> from,
	...
	// choose the rule according to the multinominal distribution defined by
	// alpha (1-alpha)
	Random r = new Random();
	...
	curRuleConfig.put(key, rulePos);// update the current rule for
					// non-terminal 'syb'
	for (String s : prods.get(rulePos)) {
	    if (nonterminals.containsKey(s)) {
		genRule(s); // the genRule would update the current rule for
			    // mutated non-term
	    }
	}
    ...
    // in order to compute the MCMC need the current state prior prob and also
    // the next state prior prob.
