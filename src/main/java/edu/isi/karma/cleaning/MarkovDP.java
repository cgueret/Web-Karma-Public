...
public class MarkovDP {
    ...
    public MarkovDP(Vector<Vector<TNode>> s, Vector<Vector<TNode>> e,
	    Vector<GrammarParseTree> gts) {
	sToks = s;
	eToks = e;
	spaces = gts;
    }
	...
	for (int cnt = 0; cnt < s.obs.size(); cnt++) {
	    ...
	    int matrix[][] = new int[a.size() + 1][b.size() + 1];// the first
								 // row and
								 // column is
								 // kept for
								 // empty
	...
	Vector<String> tmpresult = new Vector<String>(); // if no consistent
							 // intermediate rule,
							 // use the rest
	...
	for (String crule : res) {
	    // crule =
	    // "del  anytoks    from_beginning   1   from_beginning  first    Number";
	...
	for (int i = 0; i < children.size(); i++) {
		...
		double offset = Math.sqrt((Math.log(s.visitedTime)) / 5.0
			* z.visitedTime);
	...
	return vi.get(pos);
	/*
	 * Vector<Double> cv = new Vector<Double>(); double div = 0.0; for(int i
	 * = 0; i<a.size(); i++) { double c = (b.get(i)*1.0/a.get(i));
	 * cv.add(c); div += c; } //normalize double[] x = new
	 * double[cv.size()]; for(int i = 0; i<cv.size();i++) { x[i] =
	 * cv.get(i)*1.0/div; } int index = multinominalSampler(x); return
	 * index;
	 */
	...
	try {
	    ...
	    /*
	     * ResultViewer rv = new ResultViewer(); for(int z = 0 ;
	     * z<s1.size();z++) { String[] rules = s1.get(z).split("#");
	     * //System.out.println(""+s1); Vector<String> xr = new
	     * Vector<String>(); for(int t = 0; t< rules.length; t++) {
	     * if(rules[t].length()!=0) xr.add(rules[t]); } Vector<String> tres
	     * = RuleUtil.applyRuleF(xr,
	     * "/Users/bowu/Research/dataclean/data/RuleData/rawdata/50_satallite_name.txt"
	     * ); rv.addColumn(tres); }
	     */
	    ...
	    System.out.println("Time: " + (end - start) + "   Correct Result: "
		    + corrNum);
...
class MDPState {
	...
	for (int i = 0; i < obs.size(); i++) {
	    Vector<TNode> nt = RuleUtil.applyRule(rule,
		    (Vector<TNode>) this.obs.get(i).clone());
	    if (nt == null)
		return null;
	    newState.obs.add(nt);
	}
