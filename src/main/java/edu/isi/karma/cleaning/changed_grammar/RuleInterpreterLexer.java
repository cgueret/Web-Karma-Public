...
public class RuleInterpreterLexer extends Lexer {
	...
	// RuleInterpreter.g:1:8: ( ANYNUM | ANYTOK | ANYTOKS | ANYTYP | BNKTYP
	// | DEL | ENDTYP | FRMB | FRME | FST | INCLD | INS | LST | MOV | NUMTYP
	// | SRTTYP | SYBTYP | WRDTYP | TOKEN | NEWLINE | WS | NUM )
	int alt5 = 22;
	switch (input.LA(1)) {
	case 'a': {
	...
	case 'f': {
		...
		NoViableAltException nvae = new NoViableAltException("", 5, 6,
			input);
	...
	case 'i': {
		...
		NoViableAltException nvae = new NoViableAltException("", 5, 7,
			input);
	...
	case 'S': {
		...
		NoViableAltException nvae = new NoViableAltException("", 5, 11,
			input);
