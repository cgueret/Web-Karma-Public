...
public class NonterminalValidator {
    ...
    public static HashSet<String> generateWhatTemp(EditOper ops,
	    Vector<TNode> ex) {
    ...
    public static GrammarTreeNode getTemplate(String symbol, String subrule,
	    HashMap<String, GrammarTreeNode> subtemples) {
    ...
    public static void genTemplate(String symbol, String subrule,
	    HashMap<String, GrammarTreeNode> subtemples) {
    ...
    // generate the tokenspec for start end and denstination current key cannot
    // differentiate them
    ...
    public static HashSet<String> genPostion(EditOper ops, Vector<TNode> ex,
	    String type) {
    ...
    public static HashSet<String> genendendContext(EditOper ops,
	    Vector<TNode> ex) {
	HashSet<String> rules = new HashSet<String>();
	int epos = ops.endPos;
	if (epos == -1) {
    ...
    public static Vector<Vector<String>> getLiteralValue(Vector<EditOper> ops,
	    Vector<Vector<TNode>> orgexample, RuleGenerator rgen) {
	...
	Vector<String> woken = rgen.replacetokspec(
		rgen.printRules("tokenspec", (quan)), orgexample.get(0),
		ops.get(0).starPos, ops.get(0).endPos);
	wToken.addAll(woken);
	// store the context token to identify the location of a key position
	for (int k = -1; k <= 1; k++) {
	    if (pos1 != -1 && pos1 + k >= 0
		    && pos1 + k < orgexample.get(0).size()) {
		p1t.add(orgexample.get(0).get(pos1 + k).text);
	    }
	    if (pos2 != -1 && pos2 + k >= 0
		    && pos2 + k < orgexample.get(0).size()) {
		p2t.add(orgexample.get(0).get(pos2 + k).text);
	    }
	    if (pos3 != -1 && pos3 + k >= 0
		    && pos3 + k < orgexample.get(0).size()) {
	...
	for (int i = 0; i < ops.size(); i++) {
	    ...
	    Vector<String> wTokenx = rgen.replacetokspec(
		    rgen.printRules("tokenspec", (eo.tar.size())),
		    orgexample.get(i), ops.get(i).starPos, ops.get(i).endPos);
    ...
    public static Vector<String> substitueRuleVar(String rule,
	    Vector<String> newV, String tar) {
    ...
    // 4 num 5 token
    // 6 num 7 token
    public static Vector<String> assignValue(String rule,
