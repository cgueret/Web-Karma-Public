...
public class Ruler {
	...
	/*
	 * else if(input.contains("LST")) { input = input.substring(4);
	 * Vector<TNode> x = this.parsewhat(input); int pos1 = this.Search(x,0);
	 * return pos1; }
	 */
	else {
    ...
    /*
     * public int parseQuantifier(String input) { int quan =
     * Integer.parseInt(input); if(quan == -1) { return TNode.ANYNUM; } else {
     * return quan; } } public void ParseParameters(HashMap<String,String> hm) {
     * String oper = ""; int quan = 0; int startpos = -1; int endpos = 1000;
     * Vector<TNode> pat = new Vector<TNode>(); if(hm.containsKey("what")) { pat
     * = this.parsewhat(hm.get("what")); } if(hm.containsKey("operator")) { oper
     * = hm.get("operator"); } if(hm.containsKey("qnum")) { quan =
     * this.parseQuantifier(hm.get("qnum")); } if(hm.containsKey("start")) {
     * startpos = this.parseStart(hm.get("start")); } if(hm.containsKey("end"))
     * { endpos = this.parseEnd(hm.get("end")); } if(oper.compareTo("del")==0) {
	...
	/*
	 * for(int i = 0; i< operators.size(); i++) { Object[] oper =
	 * operators.get(i); if(Integer.parseInt((String)oper[2])<0) {
	 * move(Integer.parseInt((String)oper[0]), (TNode)oper[1],
	 * Integer.parseInt((String)oper[2])); } else
	 * if(Integer.parseInt((String)oper[2])>=0) {
	 * det(Integer.parseInt((String)oper[0]), (TNode)oper[1],
	 * Integer.parseInt((String)oper[2])); } }
	 */
    }
