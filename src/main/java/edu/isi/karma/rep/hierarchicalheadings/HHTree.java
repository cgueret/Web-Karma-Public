...
public class HHTree {
    ...
    public void computeHTMLColSpanUsingColCoordinates(
	    ColumnCoordinateSet ccMap, ColspanMap cspanmap) {
	for (HHTNode root : rootNodes) {
	    computeHTMLColSpan(root, ccMap, cspanmap);
	}
    }

    private void computeHTMLColSpan(HHTNode node, ColumnCoordinateSet ccMap,
	    ColspanMap cspanmap) {
	int startIndex = cspanmap.getSpan(node).getStartIndex();
	int endIndex = cspanmap.getSpan(node).getEndIndex();

	int numHtmlColsBetween = ccMap.getNumberOfCoordinatesBetweenTwoIndexes(
		startIndex, endIndex, node.getDepth());
    ...
    public void computeHTMLColSpanUsingLeafColumnIndices(
	    ColumnCoordinateSet ccMap, LeafColumnIndexMap indexMap) {
	for (HHTNode root : rootNodes) {
	    computeHTMLColSpanUsingLeafColumnIndices(root, ccMap, indexMap);
	}
    }

    private int computeHTMLColSpanUsingLeafColumnIndices(HHTNode node,
	    ColumnCoordinateSet ccMap, LeafColumnIndexMap indexMap) {
	if (node.isLeaf()) {
		...
		int c = computeHTMLColSpanUsingLeafColumnIndices(child, ccMap,
			indexMap);
		htmlColSpan += c;
	    }
	    node.setHtmlColSpan(htmlColSpan);
