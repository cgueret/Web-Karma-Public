...
public class ColumnCoordinateSet {
    ...
    private static Logger logger = LoggerFactory
	    .getLogger(ColumnCoordinateSet.class);
	...
	for (HHTNode node : spanMap.keySet()) {
	    ...
	    if (!node.isLeaf()) {
		coordinates.add(new Coordinate(span.getStartIndex(),
			Position.left, node.getDepth()));
		coordinates.add(new Coordinate(span.getEndIndex(),
			Position.right, node.getDepth()));
	    }
	}
    ...
    public int getNumberOfCoordinatesBetweenTwoIndexes(int startIndex,
	    int endIndex, int depth) {
