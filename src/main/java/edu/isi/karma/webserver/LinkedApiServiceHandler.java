...
public class LinkedApiServiceHandler extends HttpServlet {
	...
	if (resource != null
		&& resource.trim().toString().equalsIgnoreCase("input"))
	    resourceType = ResourceType.Input;
	if (resource != null
		&& resource.trim().toString().equalsIgnoreCase("output"))
	    resourceType = ResourceType.Output;

	if (format == null
		|| (!format.equalsIgnoreCase(SerializationLang.N3)
			&& !format.equalsIgnoreCase(SerializationLang.N_TRIPLE)
			&& !format
				.equalsIgnoreCase(SerializationLang.XML_ABBREV) && !format
			    .equalsIgnoreCase(SerializationLang.SPARQL)))
	...
	if (serviceId != null) {
	    new GetRequestManager(serviceId, resourceType, format, response)
		    .HandleRequest();
	} else {
	...
	if (request.getContentType().startsWith(
	    ...
	    response.getWriter().write(
		    "The content type is neither rdf+xml nor rdf+n3");
	    return;
	}

	if (format == null
		|| (!format.equalsIgnoreCase(SerializationLang.N3)
			&& !format.equalsIgnoreCase(SerializationLang.N_TRIPLE) && !format
			    .equalsIgnoreCase(SerializationLang.XML_ABBREV)))
	    format = SerializationLang.XML;
	...
	if (serviceId != null) {
	    new PostRequestManager(serviceId, in, inputLang, format, response)
		    .HandleRequest();
	} else {
	    response.getWriter().write("No Service Request ID Found!");
	}
